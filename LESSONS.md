...
- **中文 mod 名先查 MC百科再下结论（2026-08-30 阡陌乌龙）**：用户说『阡陌交通』我认成 Macaw's Paths（建材包）差点带偏方案；实际是 RoadWeaver（国创自动织路 mod，MC百科 id 22551）。用户口中的中文 mod 名是第一手线索，mcmod.cn 搜中文名直达条目，比猜英文对应快且准。
 - **RoadWeaver 已生成区块织路三板斧（2026-08-30 阡陌续篇）**：①mod 内置开关 performance.idleGenerationEnabled 默认 false——它是「服务器空闲时围绕玩家补织已生成区块」的机制，要已生成地形有路必开；②Initial 流程 COARSE_SAMPLING(CPU 模式扫千万级采样)→…→ROAD_GENERATION 漫长，等不起时走正攻：RW 卷里 roads/*.nbt 的 road.placements[].positions 就是算好的精确路面方块坐标（middle_pos[1]=路面 y，dy 全 0），python 读 nbt 生成 setblock 命令清单（每块+上方 2 格 air+每 32 段灯笼+头尾 forceload add/remove），容器内 python3 纯 socket RCON 单连接流式灌——12845 条命令 4.5 秒（rcon_flush.py）；③坑：NeoForge 1.21.1 上 datapack function 死活不注册（Unknown function，datapack list 却 enabled），别浪费时间，直接 RCON 流式灌。验证法：execute if block <路面坐标> stone_bricks 在常加载区应 Test passed；远端 forceload remove 后 not loaded 属正常（写入时区块已加载保存）。