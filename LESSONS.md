...
- commit ea45243，CI 17 绿。
  ## 2026-08-30 造物扩展（CLI 正本 + 面板 skin） - **架构定谳（造物主点破）**：底层是 CLI（/mycli），技能书/宝箱面板是给人用的前端界面——前端只把点击翻译成咒语词，裁决/计费/执行全在 CLI 层。新功能先落 CLI 动词，再套 skin。 - **造物只出面包的根因**：GIVE_WHITELIST 早有 55+ 物品且 extractParams 支持 item 参数，但书页/面板点击不带物品词 → 永远 default=bread；且 count=1 写死。修=白名单扩 76 项 + GIVE_DEFAULT_COUNT 分类数量（食物x4/原料x8-16/工具x1，护栏1-16）+ 主面板 give 格开子面板（27格物品网格，图标=物品本身）。 - **白名单镜像生成器模式**：TS 正本(GIVE_WHITELIST) → scripts/gen-give-items.mjs 生成 skill-chest.json items 段（运行卷+packaging 分发正本）→ CI 一致性测试对账（tests/js/give-whitelist-sync.test.mjs）。改 TS 必跑生成器，CI 防漂移——治五份同步顽疾的通用解。 - **改 src 后必须重启 shadow-world**：世界进程跑的是容器内 bundle，E2E 先看命令是否新语义（apple 1 vs apple 4 即旧代码信号）。 - **mycli 是玩家执行者命令**：RCON 直跑报 A player is required——面板/书页用 player.createCommandSourceStack()，命令不带玩家名参数。  - **CI 干净 checkout 四类坑（15 连红根因，2026-08-30 根治）**：本地热工作区全绿 ≠ CI 绿——①运行时文件不入库（village/config.json、mc-server/）：测试读到缺失要 skipTest 不能硬断言；②环境变量依赖（guard_drive 顶层 RCON_PASSWORD 缺失即 SystemExit）：测试 setdefault dummy；③平台差异（build.py 硬编码 javac.exe/java.exe，Linux 无）：os.name=="nt" 才拼 .exe；④node --test 目录参数 Win22/CI20 行为不一：显式列测试文件。**教训：修 CI 必须拉真实日志再修**——git credential fill 取 token 调 actions logs API（公开仓网页日志匿名拿不到），别猜。  ## 2026-08-30 神物护持（命格书防丢）+ 崩循环教训 - **build.py FULL 列表是硬编码白名单**：新增 mixin .java 必须同步进 FULL，否则 mixins.json 引用了 jar 里不存在的 class → mixin PREPARE FATAL → 服务器崩循环（本次 ~10 分钟）。修法=FULL 补两行重打包；itzg 容器 crash 自动重启会捞起新 jar，无需手动干预。长期改法：FULL 改为 glob dev/ 目录自动发现。 - **CustomData 构造私有**：1.21.1 用 CustomData.of(CompoundTag)，不能 new。 - **AutoRestore 模式（tick 节流补发）**：ServerPlayer.tick HEAD + 静态 Map<UUID,Long> 节流 100 tick，背包无 statusbook 即补——丢/死/烧/错赠全场景 5 秒回身，比事件总线（登录/重生/死亡三处挂）简单且覆盖更全。 - **DropGuard 分层**：Player.drop(ItemStack,ZZ) HEAD 拦活人丢弃（Q/拖出），!isAlive 放行死亡掉落（防 cancel 致物品凭空消失）——死亡场景由 AutoRestore 兜底。 - **RCON data modify entity 玩家 Inventory 不可用**（在线玩家 NBT 只读）；清特定 NBT 物品用 clear <player> <id>[component=~{...}] 物品谓词。 

## 2026-08-30 午 · 技能平衡批（螺旋丸准心根治 + 轮盘上线 + 构建选型坑）
- **完整 mixin 编译正牌 = settlementsfix-src\build.py**：build_settlementsfix2.ps1 三重坑——①classpath 引 mods\settlements-1.0.0-beta.1.jar 已改名 +godfix.3（取第一个非 bak settlements-*.jar 已修）；②Mojang 官方 server-*-slim/srg.jar 是 SRG/混淆名（类在方法名不在），遮蔽 neoforge-21.1.73-server.jar（patched 增量、official 名）里的 MinecraftServer.getPlayerList/getCommands——PS 端已剔壳 jar，但 **ps1 只当备用**；③javac 按时间戳增量——老 .java 靠 .class 基线通过，一旦级联重编（改 SkillChestMenu 构造）就全爆。**要动 mixin/命令类，无脑 build.py。**
- **准心方向正解：RCON 读服务端 Rotation**（getEntityNumber 'Rotation[0/1]'，度）→ dx=-sin(yaw)cos(pitch)、dy=-sin(pitch)、dz=cos(yaw)cos(pitch) → 乘速度档（慢弹 0.45≈75格/s）。bot.players 缓存缺 entity 时旧回退值写死 (1,0,1)＝「方向不可控」真根因；服务端 NBT 无约定歧义。验证=sight vector 日志与 Rotation 互算吻合+转身实时跟随。
- **沿弹伤害 = postCast 延迟命令**：atom.postCast=[{delayMs,commands}]，cast 主路径与 castByGod 双挂（setTimeout+rcon.send，vars 字符串快照复用安全）；150/450ms 两跳 damage 9 magic + 900ms 清弹防残留；弹撞方块自灭时 execute at @e[tag] 空集静默不报错。
- **damage 单实体限制的群体写法**：execute at @e[tag=弹] as @e[怪,limit=3] run damage @s —— execute as 逐体展开；直接 damage @e[limit=2] 报 Only one entity。
- **numen_act say 不进 Goddess bot 的 chat 事件**（服务器广播了、bot 无 say-heard；RCON say 正常）——假玩家公屏咏唱不能当 E2E 触发器，施法注入走 execute as <p> run mycli cast <咒>（经女神代施通道，castByGod 同渲染器同 vars）。
...
- **mod 侧数据双源四处同步**：/app/data（正本）→ 容器 /mcdata → 宿主 data/ → 宿主 mc-data/（save() 只镜像 magic-state；atoms/skill-events 手动）。
 ## 2026-08-30 羽落体系（造物主防摔死钦点）  - **御空自带缓降**：sky_walk commands 追加 effect give slow_falling 65s（60s 飞+5s 余量）——法尽坠落如落叶。atom 命令可多条顺序执行，直接叠加。 - **羽落之靴=装备化摔伤免疫**：FeatherBootsMixin 注入 LivingEntity.causeFallDamage HEAD——脚靴 custom_data:featherfall → 摔伤清零+云雾特效。装备即生效/脱下即失效。E2E：tp 150 格高落地满血。 - **词冲突坑**：atoms 匹配是子串包含且先到先得——「羽落」是「羽落之靴」前缀，短词先入库会把长咒抢走。词表设计：短词给高频应急术，复合词族要保证词根不互相包含。 - **atoms 改动须重启 shadow-world 或 reloadAtoms**：cast 用的 atoms 是启动时 loadBaseAtoms 载入内存——改文件不重启不生效（跟 magic-state 同理）。  ## 2026-08-30 自助技能附魔体系（武器主动技+装备被动光环）  - **武器技能钩子必须挂 LivingEntity.hurt 而非 Player.attack**：numen 假玩家 AI 的攻击不走 vanilla attack 交互（AttackCompanionTask 内部路径），但一切伤害必经 LivingEntity.hurt(DamageSource,float)——按 source.getEntity() instanceof ServerPlayer 判定玩家近战，覆盖真人+bot。E2E：damage 命令 10 发 4 次 proc（20% 精确）。 - **光环吸血/武器技能递归防护**：proc 内再调 target.hurt（rasengan +4）会重入 mixin——ThreadLocal reentrancy flag 必须有。 - **/mcdata/magic-state.json 会被 world 进程 save 镜像覆盖**：mod 侧校验数据（learned）直写镜像=会被洗掉。正确姿势：改正本 /app/data + 重启 shadow-world（内存重载）+ 手动补镜像 /mcdata（此后内存含新值，save 不再洗）。 - **PowerShell Set-Content -Encoding UTF8 = 带 BOM**（PS5.1），javac -encoding UTF-8 对 BOM 报诡异错位错——java 源码一律 python 重写无 BOM。 - **PS5.1 的 -replace 修 java 源码是坑**：正则元字符/编码双重风险——改源码用 python 或 edit_file，不用 PowerShell 管道。  ## 2026-08-30 施法文字缩小 + 技能瘦身  - **施法大字挡视线 → actionbar**：mc-magic 施法后处理弃用全屏 title/subtitle，改 title {target} actionbar（物品栏上方小字一行）——title 命令族里 actionbar 是唯一小字通道，粒子音效保留。 - **技能瘦身数据驱动**：skill-usage.jsonl 成功施放统计（2190 行）× atoms 全表 → 零使用名单；区分「真没用」（功能被取代：旧四光环 vs 装备光环体系、意行 vs 御空）与「够不着」（requiredLevel 高于玩家上限 lv8-10：steed 12→6、meteor 25→15 等 7 项降门槛）。砍 9 留 71。 - **砍技能前先排除新上的**（enchant/feather/aura 系上线 <1 天零使用是正常的）——瘦身要看技能年龄。 ﻿
## 2026-08-30 护花双卫 + 守卫换肤（numen_act skin）

- **RCON 客户端有 1449 字节包长上限**：mcrcon-cli/标准 RCON 库发长命令直接断连，守卫皮肤 value(408)+signature(684) 远超——长命令必须走 B 仓自研 `src/mc-rcon.ts`（分包发）。标准库「发不出长命令」不是命令错，是客户端截断。
- **brigadier string() 参数不吃裸 base64**：value/sig 含 `/+=` 等字符，裸传被 brigadier 解析器拆花——参数必须双引号包裹（`numen_act skin Kirito "<value>" "<sig>"`），引号内 base64 字符集安全。
- **numen 模块产物是 JiJ 结构**：`CompanionRegistry` 等实体类不在 core/neoforge jar 本体，而在主 jar 的 `META-INF/jarjar/numen_api-*.jar` 内嵌 jar 里（NeoForge Jar-in-Jar 运行时解包加载）。**只换 actuator jar 不换主 jar = NoSuchMethodError**（旧 api 无 snapshot()）——两个 jar 必须同批编译同批替换；排查类在哪用 zipfile 穿透 jarjar，别只看 jar 本体。
...
- **中文 mod 名先查 MC百科再下结论（2026-08-30 阡陌乌龙）**：用户说『阡陌交通』我认成 Macaw's Paths（建材包）差点带偏方案；实际是 RoadWeaver（国创自动织路 mod，MC百科 id 22551）。用户口中的中文 mod 名是第一手线索，mcmod.cn 搜中文名直达条目，比猜英文对应快且准。
 - **RoadWeaver 已生成区块织路三板斧（2026-08-30 阡陌续篇）**：①mod 内置开关 performance.idleGenerationEnabled 默认 false——它是「服务器空闲时围绕玩家补织已生成区块」的机制，要已生成地形有路必开；②Initial 流程 COARSE_SAMPLING(CPU 模式扫千万级采样)→…→ROAD_GENERATION 漫长，等不起时走正攻：RW 卷里 roads/*.nbt 的 road.placements[].positions 就是算好的精确路面方块坐标（middle_pos[1]=路面 y，dy 全 0），python 读 nbt 生成 setblock 命令清单（每块+上方 2 格 air+每 32 段灯笼+头尾 forceload add/remove），容器内 python3 纯 socket RCON 单连接流式灌——12845 条命令 4.5 秒（rcon_flush.py）；③坑：NeoForge 1.21.1 上 datapack function 死活不注册（Unknown function，datapack list 却 enabled），别浪费时间，直接 RCON 流式灌。验证法：execute if block <路面坐标> stone_bricks 在常加载区应 Test passed；远端 forceload remove 后 not loaded 属正常（写入时区块已加载保存）。 - **mod 物品装备包炸断女神化身=全施法链哑火（2026-08-30 螺旋丸修复）**：玩家装备栏带 sophisticatedbacks 背包（custom_data NBT）时，服务端发非 NeoForge 客户端的 entity_equipment 包让 mineflayer protodef 流错位（PartialReadError: Missing characters/Unexpected VarInt）→Goddess bot 断线循环（ECONNREFUSED 25599 反复）→咏唱/CLI 施法/书页点击全哑（守卫 chant 报「女神化身离线」）。修=settlementsfix 新 mixin EquipmentPacketGateForNonNeoForgeMixin：对非 NeoForge 连接把装备包里 mod 物品的 custom_data 剥掉重发净化副本（1.21.1 用 eq.getEntity() 不是 getEntityId；净化副本无 mod custom_data 天然防递归）。坑链：①「已上达女神」是 numen_act GoddessBridgeCommands 的 mycli 桥接确认语——它只是把参数私聊转发给 Goddess，bot 不在线时转发即石沉大海；②RCON 伪造的 unsigned chat 女神 bot 收不到（真客户端命令才走通），RCON 侧验证施法要用 mcdata 文件通道或真客户端；③诊断顺序：先看女神 bot 是否在线（docker logs shadow-world 搜 bot disconnected/ECONNREFUSED）再查技能状态。 - **天音炸麦根因=重采样把有符号 PCM 当无符号（2026-08-30 语音修复）**：TtsQueueWatcher.resample 里 (src[i0] & 0xFFFF) 把负样本（-32768~-1）提升成 64536+ 再削波到 32767——声音负半周全变正满幅冲击=滋啦滋啦炸麦。修=去掉 & 0xFFFF 直接用 short 算术（Java 自动有符号提升）。诊断链：SVC 连接正常+播放 done 但听感破音→先查 PCM 符号处理。另：proximity 语音 16 格限距，女神/守卫离玩家 90+ 格=完全听不见（不是故障）；从目标玩家自己实体发声音=距离 0 必达（测试用这个姿势）。
## 2026-08-30 #minecraft:monster 鏍囩骞昏锛堜簲鏀诲嚮鎶€鍏ㄥ搼锛?
- **鍧?*锛歚@e[type=#minecraft:monster]` 鍏ㄦ湇绌洪泦鈥斺€?*鍘熺増鏍规湰娌℃湁杩欎釜 entity_type 鏍囩**銆傘€屾晫瀵广€嶆槸 MobCategory 浠ｇ爜鏋氫妇锛屼笉鏄暟鎹┍鍔ㄦ爣绛俱€傜湡瀹炲瓨鍦ㄧ殑鍘熺増鏍囩锛歶ndead/arthropod/raiders/skeletons/piglin_friendly 绛夈€傛暀绋嬮噷鐨?`#minecraft:monster` 鏄互璁逛紶璁广€?- **鍒ゅ埆娉?*锛歚execute if entity @e[type=#鍙枒鏍囩,limit=1]` 鐜板満鐐瑰悕锛堥€犱竴鍙凡鐭ユ€祴锛夛紱瀵圭収 `type=minecraft:zombie`锛堝叿浣?id锛変笌 `#minecraft:undead`锛堢湡鏍囩锛夊尯鍒嗐€屾爣绛剧┖銆峷s銆岃娉曚笉鏀寔銆嶃€俁CON 鍥炴墽锛歱assed=銆孴est passed銆嶏紝failed=绌轰覆銆?- **姝ｈВ**锛歮od 鏈嶆敾鍑绘妧鐩爣閫夋嫨涓€寰嬬敤**榛戝悕鍗曟帓闄ゆ硶**锛坱ype=!player/鏉戞皯/瀹剁暅/鏉傜墿鈥︼級銆傜櫧鍚嶅崟閫愭€垪浼氭紡 mod 鎬紱鑷缓 datapack 鏍囩鍚屾牱婕忋€傞粦鍚嶅崟鎬婚暱鎺у埗鍦?~1000 瀛楃鍐咃紙RCON 璇锋眰 ~1446B 涓婇檺锛岃秴闄愰潤榛樺け璐モ€斺€旂涓€鐗?59 椤?1549 瀛楃灏辫俯浜嗭級銆?- **鐥囩姸璇嗗埆**锛氭硶鏈€屾湁闊虫晥鏃犱激瀹炽€? sounds/particles 鐙珛鎾斁姝ｅ父銆乧ommands 閲岄€夋嫨鍣ㄧ┖闆嗏€斺€斿厛鎬€鐤戦€夋嫨鍣紝鍐嶆€€鐤戜激瀹抽摼銆?- **RCON 骞跺彂涓插彴**锛氫笘鐣岃繘绋嬶紙濂崇锛変箣澶栫殑涓存椂 RCON 杩炴帴浼氭敹鍒板埆鐨勮繛鎺ョ殑鍝嶅簲锛堝搷搴旈敊浣嶏紝瀹炴祴 sel-test 鏀跺埌濂崇 statusbook sweep 鐨?JSON锛夈€備笘鐣岃繘绋嬨€宻ole RCON holder銆嶄笉鏄磥鐧栨槸蹇呴渶銆傛帓闅滆剼鏈涔堝仠涓栫晫杩涚▼锛岃涔堝蹇嶄覆鍙般€佺敤鏃ュ織楠岃瘉銆?

## 2026-08-30 guard_drive 绻佹畺骞诲奖 + 1.21 椋庡脊涓诲姩灏勭嚎 + RCON 涓插彴闃叉薄鏌?
- **venv shim 骞诲奖锛堣繘绋嬫暟鈮犲疄渚嬫暟锛?*锛歐indows 涓?`venv\Scripts\python.exe` 鏄?launcher shim锛屽惎鍔?base 瑙ｉ噴鍣紙uv cpython锛夊仛鐪熻韩骞惰浆鍙?stdio鈥斺€擟IM 閲?*涓€涓€昏緫瀹炰緥鏄剧ず涓轰袱涓?python 杩涚▼**锛坰him 鏃犲紩鍙峰懡浠よ銆佺湡韬甫寮曞彿+鍙岀┖鏍硷級銆傚垽鏂€屽嚑瀹炰緥鍦ㄨ窇銆嶅埆鏁拌繘绋嬶細鐪?**RCON ESTABLISHED 杩炴帴鏁?*鎴栭攣鏂囦欢 PID銆傚悓鐞嗚拷銆岃皝鎷夎捣浜嗚繘绋嬨€嶆椂锛宻him 鐨勫瓙杩涚▼涓嶆槸浠ｇ爜 spawn 鐨勶紝鏄?venv 鏈哄埗銆?- **msvcrt 鏂囦欢閿佷袱鍧?*锛氣憼 `open("a+")` 鍒濆 position 鍦ㄦ枃浠舵湯灏撅紝涓?`seek(0)` 灏遍攣 [len,len+1)鈥斺€斾袱涓繘绋嬮攣鐨勫尯鍩熼敊寮€锛屼簰涓嶅啿绐?閿佸け鏁堬紱鈶?閿佸啓鍦ㄦā鍧楅《灞備細鎸?`import`锛圕I test_guard_drive 鎶撳埌锛夆€斺€?*蹇呴』鍦?main() 閲屼笂閿?*銆?- **1.21 椋庡脊 = 涓诲姩灏勭嚎妫€娴嬶紝閫㈤殰鍗崇垎**锛歚breeze_wind_charge` 鐨?tick 瀵圭Щ鍔ㄥ悜閲忓仛 `ProjectileUtil.getHitResultOnMoveVector` 灏勭嚎妫€娴嬶紝瑙嗙嚎鏂瑰悜鏈夋柟鍧楋紙寤虹瓚/鏍呮爮/鏍戝彾锛夊氨 onHit 鑷垎鈥斺€擿NoPhysics:1b` 鎷︿笉浣忥紙閭ｆ槸纰版挒绠辨爣蹇楋紝灏勭嚎鐓ф牱鏌ワ級銆傞潤姝㈡棤 Motion 鍙暱鏈熷瓨娲伙紙summon 鐖嗙偣鍚堟硶鐢ㄦ硶锛夈€?*鏉戝簞/瀹ゅ唴鐜鍋氬脊閬撹瑙?*锛氳绾挎帹杩涚矑瀛愰摼锛坄execute at X positioned ^ ^1.3 ^n run particle ...` 澶氭。 delay锛? 缁堢偣闈欐 summon 鐖嗙偣锛屽埆璁╁疄浣撻琛屻€傞檮甯︽敹鐩婏細浼ゅ娌垮脊閬撴帹杩涙瘮銆岃窡闅忕煭鍛藉疄浣撱€嶆洿鍙潬銆?- **RCON 涓插彴姹℃煋瀹為獙**锛氫笘鐣岃繘绋嬬嫭鍗?RCON 鏈熼棿锛屼复鏃惰繛鎺ョ殑鍥炴墽浼氫覆锛坕f entity 鐨?passed/failed 鍙兘鏄埆浜虹殑鍝嶅簲鈥斺€斿鐓х粍鍏ㄨ姹℃煋锛屽緱鍑恒€寊+ 娲?z- 姝汇€嶇殑楝艰寰嬶級銆?*楠岃瘉瀹炰綋瀛樻椿/鍛戒护鐢熸晥鐢?`execute as @e[...] run say TAG` + docker logs**锛堟棩蹇椾笉涓插彴锛夛紝鍒俊 RCON 鍥炴墽銆?- **json 甯?BOM 鍗冲穿**锛歐indows 宸ュ叿锛圥owerShell 榛樿 utf8=甯?BOM锛夐噸鍐?`villagers.json` 鈫?瀹瑰櫒鍐?`json.load(encoding="utf-8")` 鎶?JSONDecodeError 鈫?crash loop銆傝鍏变韩 JSON 涓€寰?`utf-8-sig`锛堝甫涓嶅甫 BOM 閮藉吋瀹癸級锛涘啓鍏ョ敤 `UTF8Encoding($false)` 鏃?BOM銆?
