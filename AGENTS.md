2026-09-24 受击站定修复已部署：Fairy远程攻击被近战危险半径漏掉，具名攻击权限又错误阻断撤离；原生反射自暂停、撤离导航每20tick取消一并修复。保留快慢异步，补50ms动作入口锁等待与明确未派发回执、Jev真实bodyControl。754 Python、1068 native单测、36定向与新增真实防御GameTest通过，完整GameTest另4失败保留。实服Qwen慢任务期间4.86s撤离20.10格，吃面包独立核验6→11饥饿、原任务完成进入下一轮；仍有威胁消失后快速重开抖动，未称整体战斗全解决。Numen现役2075c5ac…，WorldBridge用新依赖真实隔离重建179项并与原JAR逐字相同后发布新record，未改旧证明哈希；survivor/导航/交互健康恢复，全局其他旧红保留。16角色/16原关闭Cron精确保持，NPC准入与桐人恢复，观察者重新入服持续跟随。runtime的livestream-ranged-defense-20260924维护已结束，勿重放；详见docs/SURVIVOR-RANGED-DEFENSE-2026-09-24.md。

2026-09-22 技能目录与Jev程序路由已部署：27初始程序经draft/test/promote加入，合计30；15支持空memory自动候选。catalog.json在草稿/晋升/共享发布原子更新，运行不扫目录，迁移/显式修复才扫描专库；精确版本/内核测试仍必验。源码与生产各246具身/34实践通过，38晋升版本200例真重测。官方jev-1.13.0实际选择base_equip_hoe，action6928bf66完成、主手木锄独立确认，practice27eed57e的evidenceComplete/objectiveObserved为true，非掌握或自主RSI收益。目录3.4s→约27ms；冷分类1376ms、接收2019ms、入队至首派发仍11s。首版扫描超时、助手回执字段断言失败保留。下载Voyager/SkillOpt/ModularRSI并适配原指南，10角色官方同步；两轮仅survivor管理器重启，10profiles/16Cron/准入恢复，15启用Cron未来班次验证。runtime/skill-catalog-20260922及handoff-fix均结束勿重放；详见docs/SKILL-CATALOG-LOOP.md。

2026-09-21 晚桐人装备停机已修：缺失铁剑经Numen同步槽派发后无RCON终态，旧网关置unknown永久暂停。现以实时背包在派发/租约消费前拒绝equipment_item_missing；真实未知仍保护。旧1de6ae97经精确原生任务与两次递增空闲身体观测操作员退役为unverified，原回执/日志保留、零重放。仅survivor管理器停启，10profiles/16Cron/准入恢复；源码/生产各226具身网关、生产34实践通过。实服task-890bcb476f0e再次缺装备被正确拒绝并继续其他工具，正常完成后进入新对话；吃饱/缺工作台/区域拒绝均非游戏成功。新增action_outcome_known健康门，相关panel通过、其他历史红项保留。同步槽通用终态及下午越界根因未整体解决。runtime/kirito-equipment-20260921已结束勿重放；见docs/AGENT-CONTINUITY-REPAIR.md。
...
2026-09-21 社交调度首阶段已部署：复用reviews.sqlite3保存版本化承诺，request_goal默认queue等整个现技能结束，replace才在原动作边界切换；after_goal_id、CAS修订、稳定ID防重，完成仅reported不作世界验收。桐人伙伴来信复用原Jev槽分类，生产socialAttentionMode默认shadow，未启用自动忽略。首次风景聊天被高置信忽略，修订18合成样例17匹配/10达阈值且正确；仍非独立准确率。复用连接17样本中位298.64ms，非端到端交流。源码/生产137具身、生产34实践、Linux235相关回归通过；实际MCP两目标入队/去重/依赖/修订/撤销已验，保留cancelled验收历史，零身体动作。仅NPC+survivor再survivor经管理器部署；此前Docker故障恢复曾重启整个容器组。10profile/16Cron/准入恢复，49/3与7/3工具、原生指南和相关panel通过，全局历史来源/资产失配保持。未实现真人统一入口/多人合批/全双工，不把旧伙伴证明改哈希冒充验收。见docs/EMBODIED-SOCIAL-SCHEDULING.md；runtime/social-scheduling-20260921维护已结束勿重放。

2026-09-21 12:10 Jev快慢控制收尾已部署：原survivor内异步单槽+持久HTTP连接，绑定目标/作业/身体并有界丢弃旧结果；补交流让位和实践局部目标优先，未接原始WASD/任务内抢占。真实影子各6样本冷816.55ms→复用278.50ms（约-66%）；实际程序一次Jev confidence0.13退回、零官方动作，HTTP613ms/主循环接收1644ms，不能宣称高频闭环或策略收益。Qwen接手真实合成木板/工作台，面包被拒，补给未完成；两新实践及指导修订失败保留。最终源码/生产116具身、生产34实践通过；此前11晋升版本74fixtures真重测。三次仅survivor经管理器重启，10profile/16Cron及准入恢复；最后临时mission恢复空值，记忆/技能未回滚，自主已恢复。48/3与7/3工具可用、指南官方同步、相关panel通过，全局旧来源/资产问题保留。见docs/JEV-FAST-SLOW-CONTROL.md；runtime/jev-fastslow{,-fairness,-goal}-20260921一次性维护均结束，勿重放。
