...
控制器每 15 秒观察身体与事件、每 60 秒刷新周边，模型决策至少间隔 180 秒，滚动 24 小时最多 96 轮（本轮功能验证提高上限，保留原用量）。新目标、世界事件、生命/饥饿/库存变化、明显位移或动作结果可触发下一轮；持续模式也按模型安排的 180–3600 秒间隔复盘，默认 1800 秒，短目标完成后继续提出下一目标。平静期间进入 `observing`，没有事件也会在下次复盘继续；`idle` 仍可用于单任务模式。复盘不会绕过暂停、不确定结果或预算。
