...
def tick(c, body, control):
    ...
    try:
        ...
        if (not allowed or c.pending_policy is not None or getattr(c, 'pending_route', None) is not None
                or getattr(c, 'pending_motor', None) is not None or c.data.get('dialogueActive')
