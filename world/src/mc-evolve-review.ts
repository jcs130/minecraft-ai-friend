...
export function createEvolveReview(config: Config, deps: EvolveReviewDeps): EvolveReviewHandle {
  ...
  /** Existing role/session and 120-second limit; no publication policy changes. */
  async function askGoddess(prompt: string): Promise<string> {
    return (await modelProvider.chat({ roleId: 'mc-god', sessionId: 'mc:evolve:review', userId: 'evolve-review',
      prompt, timeoutMs: 120_000 })).text
  }
