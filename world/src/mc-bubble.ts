...
export function createBubble(config: Config, deps: BubbleDeps): BubbleHandle {
  ...
  async function goddessWelcome(player: string) {
    try {
      const title = JSON.stringify({ text: '千灯纪', color: 'gold', bold: true })
      const subtitle = JSON.stringify({ text: '✦ 女神守望 · 没有魔王，只有灯火 ✦', color: 'yellow' })
      ...
      for (const line of lines) {
        await rsend(`tellraw ${player} ${JSON.stringify(line)}`)
