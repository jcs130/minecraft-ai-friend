...
import { Rcon, RconNotSentError } from './rcon.ts'
...
export function createRcon(config: Config): RconHandle {
  ...
  async function ensure(): Promise<Rcon> {
    if (disposed) throw new Error('rcon service disposed')
    ...
    connecting = (async () => {
      ...
      try { await conn.connect() } catch (error) { conn.close(); throw error }
      if (disposed) { conn.close(); throw new Error('rcon service disposed') }
      rcon = conn
      log('rcon connected')
      return conn
    })()
  ...
  const service: RconService = {
    send: async (cmd) => {
      // Keep one command in flight on Minecraft's persistent RCON stream.
      // A failed request cannot close a socket another caller is already using.
      const result = queue.then(async () => {
        ...
        try {
          ...
          // EPIPE, reset, close and timeout after write are ambiguous: the server
          // may have executed the command. Never replay gifts/effects on those errors.
          if (error instanceof RconNotSentError) return (await ensure()).send(cmd)
          throw error
        }
      })
      queue = result.then(() => {}, () => {})
