...
export function createSaga(config: Config, deps: SagaDeps): SagaHandle {
  ...
  // Same existing role/session/prompt; provider owns the transport.
  async function callGoddess(prompt: string): Promise<string> {
    return (await modelProvider.chat({ roleId: 'mc-god', sessionId: 'mc:saga', userId: 'saga',
      prompt, timeoutMs: 180_000 })).text
  }
