...
import { cliOverview } from './mc-cli.ts'
...
export function welcomeLines(name: string): string[] {
  return [
    `旅人 ${name}，欢迎降临千灯纪。施法就一句：私聊女神 /msg Goddess 念法术关键词（归乡/造物/烟花术，或铁魔法：法术名），按原生装备与秘术规则施放。`,
    `查状态打 /mycli status；选天赋喊「我选 <法术名>」；求助说「祈愿：…」；有疑问说「问：…」；更多帮助看 /myhelp。`,
  ]
}
