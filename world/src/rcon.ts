...
/** Only this error proves that send() did not write a command to the socket. */
export class RconNotSentError extends Error {
  constructor() { super('rcon not connected before send'); this.name = 'RconNotSentError' }
}
...
export class Rcon {
  ...
  constructor(
    ...
    private readonly maxCommandBytes = Number(process.env.MC_RCON_MAX_COMMAND_BYTES || 1446),
  ) {
    if (!Number.isInteger(maxCommandBytes) || maxCommandBytes < 1446 || maxCommandBytes > 65526) {
      throw new Error('MC_RCON_MAX_COMMAND_BYTES must be an integer from 1446 to 65526')
    }
  }

  isConnected(): boolean {
    return this.connected && this.socket !== null && !this.socket.destroyed &&
      this.socket.writable && !this.socket.writableEnded
  }
  ...
  connect(timeoutMs = 6000): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.on('data', (chunk) => { if (this.socket === socket) this.onData(chunk as Buffer) })
      socket.on('close', () => {
  ...
  send(command: string, timeoutMs = 8000): Promise<string> {
    if (!this.isConnected()) return Promise.reject(new RconNotSentError())
    // Unpatched Minecraft 1.21.1 reads a maximum 1460-byte frame (14-byte overhead).
    // A verified server framing fix can opt into a larger bound. Never truncate commands.
    if (Buffer.byteLength(command, 'utf8') > this.maxCommandBytes) {
      return Promise.reject(new Error(`rcon command exceeds configured limit (${this.maxCommandBytes} UTF-8 bytes)`))
    }
    const id = this.nextId++
    return new Promise((resolve, reject) => {
