...
import {
  parseCli, parseBareCli, explicitChantBody, cliOverview, shapeStatus,
  type CliCommand,
} from './mc-cli.ts'
import { WaypointStore, zhNumberToArabic, type Waypoint } from './mc-waypoints.ts'
import { createLifecycle } from './lifecycle.ts'
import { createWorldModelProvider } from './providers/world-model-provider.ts'
import { describeModelProvider } from './providers/provider-info.ts'
import type { ModelProvider, ModelProviderInfo, ModelReply } from './providers/model-provider.ts'
import { parseNbtPosition } from './mc-nbt.ts'
import { createSkillCliQueue } from './skill-cli-queue.ts'
import { createIronsSpellClient, NATIVE_SPELL_ID } from './irons-spell-client.ts'
import { queryNativeProgression, nativeProgressionLines } from './native-progression.ts'
import { incomingWhisper } from './incoming-whisper.ts'
import { privateFeedbackCommand } from './cli-feedback.ts'
import { createWaypointTravel } from './waypoint-travel.ts'
import { hasSkillCatalog, newcomerSkillGuidance, nativeEmergencyHealingFeedback } from './catalog-guidance.ts'
...
function verdictPrompt(
  ...
  isVillager = false,
  vip = false,
  catalogMode = hasSkillCatalog(atoms),
): string {
  const lines = [
    `【祈愿】${senderName}：${wish}`,
  ]
  ...
  if (isVillager) {
    ...
    lines.push(catalogMode
      ? '- 村民优先给安全指引与白名单物资建议；只能讨论上表当前精选，不能授予归档技能或凭空解锁原生法术。'
      : '- 神恩有价：村民须舍贡（祈愿里自陈的麦/鱼/炭等即其供奉），神多指引、少代劳——对村民优先给指引（告诉他怎么自渡），不要代施神迹（他不能咏唱，代施意义也异于穿越者）；天象级恩典（破晓/驱云等全村同享）可酌情；')
    ...
    lines.push(catalogMode
      ? '- 技艺只能选上表当前精选；归档旧术不可再施。原生法术须由玩家实际装备的法术书或卷轴按原生法力、冷却施放，不能用神迹绕过。'
    ...
    if (vip) {
      ...
      lines.push(catalogMode
        ? '- 若她/他处境艰难，可按上表精选提供归乡、造物等帮助或安全撤退指引；治疗只提示实际装备的原生治疗法术，不许声称已经救活。'
        : '- 若她/他处境艰难（重伤/濒死/被困/迷失/身无长物），可主动代施辅助法术（如圣愈、归乡、照明）或赐予白名单内物资，或在 reply 里给明晰指引；')
      lines.push(catalogMode
        ? '- 代施与赐物仍守精选目录、白名单与数量边界；归档技能与未装备的原生法术不能因 VIP 身份绕过。'
        : '- 代施/赐物仍守白名单与数量克制；天象级恩典（破晓/驱云）仍慎用，全服同享不可私赏。')
    }
  }
  return lines.join('\n')
}
...
export interface GodService {
  /** Public metadata from the selected runtime provider, without endpoint or credentials. */
...
export function createGod(config: Config, deps: GodDeps): GodHandle {
  ...
  const cliWhisper = (target: string, text: string) => {
    try {
      const command = privateFeedbackCommand(getBot()?.username ?? '', target, text)
      rcon.send(command).catch(() => log('CLI private feedback unavailable'))
    } catch { log('CLI private feedback target or message invalid') }
  }
  const magic = deps.magic
  const availableLegacyAtoms = () => magic.listAtoms().filter(a => a.type !== 'passive' && (!a.catalog || a.catalog.status === 'featured'))
  ...
  async function tpWaypoint(username: string, wp: Waypoint) {
    ...
    const result = await waypointTravel.teleport(login, wp)
    ...
    return { ...result, waypoint: wp, summary: `✦ 已抵达「${wp.name}」（${result.dimension} · ${Math.round(result.x!)}, ${Math.round(result.y!)}, ${Math.round(result.z!)}）。` }
  }
  ...
  async function handleNumberTeleport(username: string, message: string): Promise<boolean> {
    ...
    if (r.ok) lastTp.set(username, now)
    cliWhisper(username, `[信使] ${r.summary}`)
  ...
  async function askGoddess(username: string, senderName: string, wish: string, offering: OfferingInfo | undefined, isVillager = false, vip = false): Promise<Verdict> {
    ...
    const atoms = availableLegacyAtoms()
    ...
    let prompt = verdictPrompt(senderName, wish, atoms, snapshot, offering ?? undefined, worlddb.ledgerSummary(username), memories, isVillager, vip, hasSkillCatalog(magic.listAtoms()))
    ...
    const validIds = new Set(availableLegacyAtoms().map((a) => a.id))
  ...
  async function initNewcomer(username: string): Promise<void> {
    ...
    const prompt = [
      ...
      '1. 一句欢迎（点明他刚醒来、这是千灯纪）。',
      newcomerSkillGuidance(magic.listAtoms()),
  ...
  async function answerQuestion(username: string, question: string, replyTarget?: string): Promise<void> {
    ...
    try {
      ...
      const atomLines = availableLegacyAtoms().map((a) => {
      ...
      const playbook = [
        '【世界玩法要诀（本神亲订，作答权威依据）】',
        '旧技能已精简；下面只列当前特色秘术。战斗、治疗和常见增益用已装备的铁魔法（/mycli spells），不能凭名字解锁或发书。旧学会记录只是档案，不能据此承诺旧主动技能仍可施放。',
  ...
  async function handleExtendedPlayerCommand(subject: string, replyTarget: string, cmd: CliCommand, isGuardian = false,
  ...
  const chantingStaff = createChantingStaffClient(rcon)
  const playerCommands = createPlayerCommands({
    magic, irons, rcon, worlddb, waypoints, waypointTravel, tpWaypoint, resolveLogin,
    skillBookItem, parseNbtPosition, queryNativeProgression, nativeProgressionLines,
    cliWhisper, bubble: deps.bubble, now: Date.now,
    getCultivationCooldowns: () => (globalThis as any).__cultCd ?? ((globalThis as any).__cultCd = new Map<string, number>()),
  ...
  const pendingSpokenConversation = new Set<string>()
  const spokenCommands = createSpokenCommands({
    atoms: () => magic.listAtoms(), execute: playerCommands.executeRequest,
    prepareCast: chantingStaff.claimVoice,
    feedback(actor, text) { cliWhisper(actor, `[言灵] ${text}`); speakViaGodVoice(text, actor) },
  ...
  const voiceInbox = createVoiceCommandInbox({
    directory: join(process.env.GODVOICE_DIR || join(DATA_DIR, 'godvoice'), 'mic', 'outbox'),
    receipts: join(DATA_DIR, 'voice-command-receipts.jsonl'),
    allowed: new Set((process.env.VOICE_ALLOWED_PLAYERS || 'MengMeng').split(',').map(name => name.trim()).filter(Boolean)),
    online: () => new Set(Object.keys(getBot()?.players ?? {})), now: Date.now,
    execute: spokenCommands.execute,
    observed(actor, text) {
      // JSON component escaping preserves punctuation/negation in the actual
      // transcript; never strip quotes before parsing the player's intent.
      void rcon.send(`title ${actor} actionbar ${JSON.stringify({ text: `听到：${text.slice(0, 100)}`, color: 'aqua' })}`).catch(() => undefined)
      deps.bubble?.show(actor, `「${text.slice(0, 60)}」`)
    },
    log,
  })
  // Register once per world-service instance; reconnecting Goddess must not
  // accumulate microphone scanners. Online identity is checked for each input.
  lc.setInterval(() => { void voiceInbox.poll() }, 500)
  ...
  const pollSkillQueue = () => {
    ...
    lc.setTimeout(() => {
      skillQueue.poll().catch(e => log(`skill CLI poll: ${e instanceof Error ? e.message : String(e)}`)).finally(pollSkillQueue)
    }, 200)
  }
    ...
    try {
      ...
      for (const ln of lines) {
        try {
          ...
          if (!magic.sniffChant(trimmed) && explicitChantBody(trimmed) === null && !NATIVE_SPELL_ID.test(trimmed)) {
  ...
  async function handleOne(item: InboxRow): Promise<void> {
    ...
    if (ms.hpRatio !== null && ms.hpRatio <= 0.4) {
      const surv = emergencySurvivalSpell(wish, availableLegacyAtoms())
      if (surv) {
        try {
    ...
    const instant = planInstantReply(wish, { mana: ms.mana, learned: ms.learned, innateSkill: ms.innateSkill }, availableLegacyAtoms(), 2.0)
    if (instant) {
   ...
   * 原生等级只读；附魔、铁砧和等级命令可能使 XpTotal 与当前等级不一致。
   * 总量差异只作限频诊断，不能据此返还花掉的等级或重写玩家进度。
   */
  ...
  async function syncLevel(name: string): Promise<void> {
    ...
    if (xp === null) return
    // 附魔/铁砧正常花费等级不会同步扣减 XpTotal；保留观测而不写回原版经验。
    if (total !== null) {
      const impliedMin = totalForLevel(xp)
      const overBy = impliedMin - total
      const underBy = total - (impliedMin + xpToNext(xp))
      if (overBy > 250 || underBy > 250) {
        const now = Date.now()
        const last = lastXpDiagnostic.get(name)
        if (last === undefined || now - last >= 300_000) {
          lastXpDiagnostic.set(name, now)
          log(`XP DIAGNOSTIC ${name}: XpLevel ${xp}, XpTotal ${total}; level spending or commands may explain this difference; native level unchanged`)
        }
      }
    }
    ...
    if (prev !== undefined && xp > prev) {
      ...
      const unlocked = magic.listAtoms()
        .filter((a) => a.requiredLevel > prev && a.requiredLevel <= xp && (!a.catalog || a.catalog.status === 'featured'))
        .map((a) => a.name)
      try {
  ...
  // 目录模式：濒死仅尝试正常原生治疗；受理不代表恢复生命，失败不写成功史记。──
  ...
  async function guardScan(name: string, isNight: boolean, day: number): Promise<void> {
    try {
      ...
      const catalogMode = hasSkillCatalog(magic.listAtoms())
      log(`GUARD-SCAN ${name}: hp=${hp} food=${food} air=${air}`) // AUDIT-02 调试：确认扫描取数
      if (hp !== null) {
        const hpRatio = hp / 20
        if (hpRatio <= 0.15) {
          // The equipped native source owns mana/cooldown/cast time. Never call
          // both engines or turn a rejected archive spell into a rescue success.
          let dyingHint = catalogMode ? nativeEmergencyHealingFeedback().hint : '我已出手救你（圣愈）——别硬撑，快念「圣愈」自保，或速撤找屋檐！'
          const sinceSave = lastGuardSave.get(name) ?? 0
          if (Date.now() - sinceSave > GUARD_SAVE_COOLDOWN_MS) {
            lastGuardSave.set(name, Date.now())
            try {
              if (catalogMode) {
                const result = await irons.cast(name, 'irons_spellbooks:heal')
                const feedback = nativeEmergencyHealingFeedback(result)
                dyingHint = feedback.hint
                log(`GUARD-SAVE ${name}: native heal (hp ${Math.round(hp)}/20) -> ${result.code}`)
                if (feedback.record) {
                  worlddb.chronicleRecord('verdict', name, feedback.record)
                  await worlddb.remember(name, 'verdict', `守卫${name}濒死（${Math.round(hp)}/20）；原生治疗已开始施法，尚未确认生命恢复。`)
                }
              } else {
              ...
              if (catalogMode) dyingHint = nativeEmergencyHealingFeedback({ ok: false, code: 'outcome_unknown' }).hint
              log(`guard save failed for ${name}: ${err instanceof Error ? err.message : String(err)}`)
            }
          }
          await guardHint(name, 'dying', dyingHint)
        }
          ...
          if (catalogMode) {
            await guardHint(name, 'starving', '你正空腹失血，立即撤到安全处进食；物资不足可用 /mycli cast give item=bread count=4，先查看状态确认魔力。')
          } else {
            const sinceSave = lastGuardSave.get(name) ?? 0
            if (Date.now() - sinceSave > GUARD_SAVE_COOLDOWN_MS) {
              lastGuardSave.set(name, Date.now())
        ...
        else if (hpRatio < 0.30) await guardHint(name, 'hurt', catalogMode
          ? `你伤得不轻（${Math.round(hp)}/20），先撤到安全处进食；装备治疗法术书或手持卷轴后，用 /mycli spells 查看原生治疗。`
          : `你伤得不轻（${Math.round(hp)}/20），找屋檐歇脚，或念「圣愈」/向女神求个恩典。`)
      }
      ...
      if (Date.now() - sinceDisc > EXPLORE_COOLDOWN_MS) {
        ...
        const pm = parseNbtPosition(posRaw || '')
        if (pm) {
          const [x, y, z] = pm
  ...
  function ensureAvatar(bot: Bot) {
    ...
    bot.on('chat', (username: string, message: string) => {
          ...
          if (wp) {
            ...
            if (now - (lastTp.get(username) ?? 0) >= 3000) {
              tpWaypoint(username, wp)
                .then((r) => { if (r.ok) lastTp.set(username, now); cliWhisper(username, `[信使] ${r.summary}`) })
                .catch(() => { /* */ })
              log(`number-tp ${username} -> ${n}:${wp.name}`)
            }
      ...
      if (isHelpCommand(message)) {
        ...
        if (lines) for (const ln of lines) cliWhisper(username, `[手册] ${ln}`)
      ...
      if (isCliCommand(message)) {
        ...
        for (const ln of lines) cliWhisper(username, `[手册] ${ln}`)
    ...
    // One incoming stream handles both signed and profileless private messages;
    // the server registry identifies its format and online profiles identify its sender.
    ;(bot as any)._client?.on('playerChat', (data: any) => {
      const incoming = incomingWhisper(data, (bot as any).players ?? {}, (bot as any).registry?.chatFormattingById)
      if (!incoming) return
      handleWhisper(incoming.username, incoming.message).catch((err) => log(`handleWhisper failed for ${incoming.username}: ${err instanceof Error ? err.message : String(err)}`))
    })
  }
  ...
  if (config.enabled) {
    // Start only after all createGod state (including disposed) is initialized.
    pollSkillQueue()
  ...
  const service: GodService = {
    providerInfo: () => describeModelProvider(modelProvider),
