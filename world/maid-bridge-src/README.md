...
2026-09-14：结衣的正常对话已增加持久感知收件协议。精确 Yui/Kirito 身体与实际 Numen 主人、正常 `LLMCallback.class` 才发送 `qd_delivery: perception_queue_v1`，仅携带最后一条原生用户消息，不重复历史。独立8容量HTTP通道等待15秒收件，严格验证202的七字段回执后只删除等待气泡，不添加助手历史或TTS。Python持久收件由原十分钟生活信号消费；不立即唤醒、不自动广播私有内容。其他人物、设定生成与历史摘要仍按下文原200同步协议处理。完整状态及边界见 [结衣对话收件箱](../../docs/YUI-DIALOGUE-INBOX.md)。
