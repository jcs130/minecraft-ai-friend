...
description: 在千灯纪角色反复遇到同类任务、收到失败反馈或开始定时复盘时，创建、验证、试用和回滚本角色流程技能；按需参考技能市场。
---
...
`learning_schedule()` 查看自己的每周维护任务；可编辑 enabled、weekday、hour，不能新增高频 cron。运营周复盘只有存在待改进事实时才进入原共享 4 次/24 小时与 30 分钟冷却预算；游戏周维护只做本地检查并留待下次正常对话学习，不启动第二条生存思考循环。heartbeat、模型重试及其他角色仍受原配置约束。
