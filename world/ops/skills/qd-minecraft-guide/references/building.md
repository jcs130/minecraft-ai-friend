...
先读当前背包、任务与可用储物点，自主决定需要保留、存放或让出的物品；不要为了腾格反复在营地堆无用途的方块。`drop_items(turn_id, item_id, count)` 可将主背包中的 1–64 件物品按当前面朝方向原生丢到地上，保留名称、附魔、耐久及模组组件。该工具目前仅限直接动作，不在旧程序技能内核中。

返回成功时会给出实际掉落实体、数量及库存减少的原生回执；地面丢出不是长期储物，物品之后仍可能被拾取、损毁或消失。给附近结衣等队友递物资时，通过游戏交流并另查她的实际背包/拾取回执，不能把“已丢出”说成“队友已收到”。未知结果不重发原操作；保持原请求编号，等待核对。
