...
先查 `team_cases(owner="mine")` 或需要时的 `owner="all"` 短索引，选本轮要推进的一项，再用 `team_case(case_id)` 读取当前详情、最新 `version` 和最近 3 条完整事件。返回 `has_more=true` 时，只有核实本事项确实需要旧证据，才把 `next_before_seq` 传入 `before_seq` 读取更早一页；`event_limit` 可取 1–20。每页按时间顺序返回，全部原文审计保留，不必先读完整历史或展开其它工单。快照过期、角色暂未观测到、执行失败是不同情况；引用快照的时间与字段，不从缺失直接推断死亡或服务崩溃。
