...
| 回执 | 含义与下一步 |
| --- | --- |
| queued / claimed / busy | 请求仍在原队列；用同 ID 的 receipt 等待。短暂排队不是队列死锁。 |
| unknown / outcome_unknown / 连接中断 | 结果未明；只核原 ID，不换名再提交，不声称失败或成功。 |
| rescue_observation_expired | 本次救援已明确拒绝；旧观察不能使用。若当前仍有独立救援必要，先用新 ID 获取新鲜观测，再重新决定。 |
| quote_source_moved | 被观察身体已经移动，本次救援未执行；先核实现在是否已自行脱困或回营地，不追着移动角色反复传送。仍必要时重新观察。 |
