...
# Explicit isolated mount only; no production-directory fallback.
ROOT = voice_root()
...
TTS_LOCAL_URL = local_tts_url(os.environ.get("TTS_LOCAL_URL", ""))
ALLOW_EDGE_FALLBACK = os.environ.get("VOICE_ALLOW_EDGE_FALLBACK", "0") == "1"
        ...
        try:
            ...
            for f in sorted(TEXT_Q.glob("*.json")):
                ...
                try:
                    ...
                    jid = job_id(job.get("id") or f"{int(time.time()*1000)}")
                    ...
                    try:
                        ...
                        if not ALLOW_EDGE_FALLBACK:
                            raise RuntimeError('Local TTS failed; cloud fallback is disabled') from le
                        log(f"tts-local miss {jid}; fallback edge-tts")
