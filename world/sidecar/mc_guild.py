...
BASIC_QUESTS = os.environ.get("NPC_GUILD_BASIC_QUESTS", "1") == "1"
...
def rank_idx_of(fame):
    return Rules.rank_idx_of(fame, RANKS)
...
def build_treasure(x, y, z):
    ...
    if not N.GUILD_AUTOGENERATE:
        raise RuntimeError("Guild world generation is disabled")
...
def pick_visits(day):
    out = []
    # Legacy fixed landmarks belong to an earlier map; basic mode only offers
    # an actual distance check until those landmarks have been verified.
    spots = [s for s in VISIT_SPOTS if s["key"] != "far_horizon"] if N.GUILD_AUTOGENERATE else []
    chosen = spots[:1]
...
# ---------------- 看板生成 ----------------
def gen_board(day):
    # Repeated generation and midnight refresh must preserve claimed boards.
    if os.path.exists(guild_path(day)):
        with open(guild_path(day), encoding="utf-8") as handle:
            return json.load(handle)
    if not BASIC_QUESTS and not N.GUILD_AUTOGENERATE:
        return {"date": day, "board": []}
...
def _zh_num(s):
    return Rules.zh_num(s)

def _takers(b):
    return Rules.takers(b)
...
def _rank_gate(who, b):
    ...
    if need <= 0:
        return None
    fm = load_fame()
    return Rules.rank_gate(b, fm.get(who, {}).get("fame", 0), RANKS)

def _gather_matches(b):
    """Imported qids alone are insufficient: daily quest goods can differ."""
    try:
        return Rules.gather_matches(b, N.quests_today()["quests"])
    except Exception:
        return False


def _is_far_horizon(b):
    # Older boards dropped `spot` while copying the visit template. Recognize
    # only the exact legacy distance quest; never reinterpret another landmark.
    return Rules.is_far_horizon(b)
...
def my_lines(who):
    doc = board_today()
    return Rules.my_lines(who, doc["board"])

def fame_lines(who):
    fm = load_fame()
    rec = fm.get(who)
    return Rules.fame_lines(who, rec, RANKS)
...
def claim(who, no):
    ...
    baseline = hunt_score(who, b["mob"]) if b["type"] in ("hunt", "boss") else None
    if b["type"] in ("hunt", "boss") and baseline is None:
        return ["暂时读不到讨伐记录，尚未接单；请稍后再试。"]
    b["status"] = "claimed"
    b["taker"] = [who]
    b["taken_at"] = time.strftime("%H:%M")
    if b["type"] in ("hunt", "boss"):
        b["baseline"] = {who: baseline}
    save_board(doc)
...
def party_join(who):
    ...
    baseline = {}
    if b["type"] in ("hunt", "boss"):
        baseline = {leader: hunt_score(leader, b["mob"]), who: hunt_score(who, b["mob"])}
        if any(value is None for value in baseline.values()):
            return ["暂时读不到队员讨伐记录，尚未组队接单；请稍后再试。"]
    b["status"] = "claimed"
    b["taker"] = [leader, who]
    b["taken_at"] = time.strftime("%H:%M")
    if b["type"] in ("hunt", "boss"):
...
def _write_health(doc, error=None):
    """Publish bounded, player-free evidence that the guild poller is alive."""
    global _HEALTH_LAST_SUCCESS
    now = time.time()
    if error is None:
        _HEALTH_LAST_SUCCESS = now
    counts = {}
    for task in (doc or {}).get("board", []):
        kind = task.get("type", "unknown")
        counts[kind] = counts.get(kind, 0) + 1
    record = {"schema": 1, "updated_at": now, "last_success_at": _HEALTH_LAST_SUCCESS,
              "poll_interval_seconds": GCFG.get("tick_interval", 30),
              "basic_quests": BASIC_QUESTS, "autogenerate": N.GUILD_AUTOGENERATE,
              "board_date": (doc or {}).get("date"), "task_counts": counts,
              "error_type": type(error).__name__ if error is not None else None}
    target = os.path.join(DATA, "guild-health.json")
...
def hunt_score(who, mob):
    ...
    try:
        ...
        m = re.search(r"\bhas\s+(-?\d+)\s+(?:\[|score)", r)
        if m:
