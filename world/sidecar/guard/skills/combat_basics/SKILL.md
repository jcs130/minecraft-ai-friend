...
## Spell casting in Qiandengji

Use `goddess_cli("spells")` to inspect the spells in your equipped Iron's spellbook or held scroll, and `goddess_cli("status")` for native mana, casting state, attributes and Pufferfish progression. Cast an exact returned ID with `goddess_cli("cast irons_spellbooks:firebolt")`. Your aim, native equipment, resources and cooldown still apply. `goddess_cli("cancel")` explicitly interrupts a cast.

`casting_started` acknowledges native acceptance, not a hit or completed effect. Check status before the next action. For `pending` or `outcome_unknown`, use `skill_receipt(request_id)` and inspect the body; never resubmit a resource-consuming action automatically.

`goddess_cli("spells legacy")` lists eight featured legacy abilities: `home`, `tp`, `give`, `blood_mana`, `spring`, `sky_walk`, `feather_boots`, and `fireworks`. For example, `goddess_cli("cast tp distance=5 direction=东")` uses the existing legacy mana ledger; the native spell bridge never charges that ledger as an extra spell cost. `blood_mana` converts health to legacy mana only. `feather_boots` creates wearable boots; it is not a passive-unlock operation. Persistent waypoint travel remains a separate entry.

`goddess_cli("spells archive")` shows 64 historical entries: 57 old active abilities now reject casting with `skill_archived`, while seven passive definitions retain their existing progress/rewards. Do not cast old `feather_fall`, `heal`, `chain_lightning`, `rasengan`, or special `fire_aura`, and do not try alternate spellings or guardian/admin commands to bypass the archive. Existing learned/innate history, passive benefits, enchanted gear, weapon hit effects and armor auras are not stripped. A nativeHints alternative is a suggestion, not an unlock or an automatic equipment grant: obtain and equip the real source, then use the exact ID returned by `spells`. Do not use admin casting commands or guess an unlisted native spell.
