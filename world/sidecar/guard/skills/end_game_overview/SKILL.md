...
1. **Use 你的 goal(阶段清单记在 goal 里)** to write the 6 mainline phases as a top-level todo list, phase 1 `in_progress`.
2. **Load the matching skill** with `read_skill` only when you actually start that phase — loading all skills up front wastes tokens.
...
Fresh world, no gear: `read_skill("tier_progression")`.
