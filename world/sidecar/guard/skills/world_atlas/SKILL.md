---
name: world_atlas
description: Vanilla Minecraft 1.21.1 structure and biome reference with registry ids, landmarks, dangers, loot, family tags, and structure-versus-biome routing. Modded content is additional. Load before locate_structure / locate_biome.
---

# Skill: world_atlas

`locate_structure` / `locate_biome` take EXACT registry ids (or `#tags`),
current dimension only. This reference targets Minecraft 1.21.1. It covers
the vanilla locations below; the server's mods and enabled data packs can add
more registry ids. An id missing from this guide is not proof that it is absent
from this server. Check the installed content and the locate tool response.
