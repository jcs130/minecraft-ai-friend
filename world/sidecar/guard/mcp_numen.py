...
REPO_ROOT = os.path.abspath(os.environ.get("NUMEN_REPO_ROOT") or
...
DATA = os.environ.get("NUMEN_DATA_DIR") or os.path.join(REPO_ROOT, "data")
...
async def goddess_cli(command: str) -> str:
    """技能 CLI：spells 查看已装备铁魔法，cast irons_spellbooks:firebolt 原生施法，cancel 停咒，status 查原生属性/成长；旧秘术查 spells legacy，cast feather_fall。支持help/skills/skillbar/menu。返回实际JSON回执；casting_started只代表已接纳，pending/outcome_unknown须用skill_receipt查requestId，不自动重放。"""
    ...
    result = await asyncio.to_thread(request_skill, WORLD_DATA, NUMEN_COMPANION, command)
    return json.dumps(result, ensure_ascii=False)


@mcp.tool()
async def skill_receipt(request_id: str) -> str:
    """读取技能 CLI 的既有执行回执，不提交新施法。用于 pending/outcome_unknown 后查询。"""
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__))) if os.path.dirname(os.path.abspath(__file__)) not in sys.path else None
    from skill_cli_client import read_receipt
    return json.dumps(read_receipt(WORLD_DATA, request_id), ensure_ascii=False)
...
SKILLS_DIR = os.path.abspath(os.environ.get("NUMEN_SKILLS_DIR") or
                             os.path.join(REPO_ROOT, "sidecar", "guard", "skills"))
