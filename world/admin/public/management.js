(() => {
  ...
  function renderServices(){
    const signature=JSON.stringify([authenticated,services]);if(signature===serviceSignature)return;serviceSignature=signature;
    ...
    $('managed-services').replaceChildren(...services.map(row=>{
      ...
      for(const [action,label] of [['start','启动'],['restart','重启'],['stop','停止']]){const b=make('button','button button-secondary compact',label);b.type='button';b.disabled=!authenticated||!row.canControl||(action==='start'&&row.state==='running')||(action==='stop'&&row.state==='exited');b.onclick=()=>preview(action,row.id);actions.append(b);}
  ...
  async function preview(action,id){try{message('');plan=await api('/api/manage/plan',{action,services:[id]});
  ...
  $('plan-cancel').onclick=()=>$('operation-dialog').close();
  $('plan-execute').onclick=async()=>{if(!plan)return;$('plan-execute').disabled=true;try{const result=await api('/api/manage/execute',{planId:plan.id});$('operation-dialog').close();message('维护已提交，正在等待实际执行结果。记录编号 '+result.operationId.slice(0,8));plan=null;await refresh();}catch(e){$('operation-dialog').close();message(e.message,true);}};
