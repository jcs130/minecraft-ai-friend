import {buildDecisionStreamGraph} from './decision-stream-model.js';
...
function status(){const stale=failed||trace?.stale||!trace?.available;document.body.classList.toggle('updates-paused',paused||stale||document.hidden);$('stream-status').classList.toggle('warn',stale||paused);$('stream-status').textContent=paused?'更新已暂停':failed?'读取失败 · 保留快照':!trace?.available?'暂无决策数据':trace.stale?'来源快照过期':'● 快照已连接';$('stream-time').textContent='快照 '+format(trace?.generatedAt);}
...
function render(){status();const next=buildDecisionStreamGraph(failed?{...trace,stale:true}:trace),key=JSON.stringify({...next,at:null});
