(() => {
  ...
  function actions(){return (trace?.turns||[]).flatMap(t=>t.actions).sort((a,b)=>(b.startedAt||0)-(a.startedAt||0));}
  function connection(){
  ...
  function draw(){
    ...
    $('current-tool-info').textContent=last?`${last.tool} · ${last.status==='completed'?'执行结束':statuses[last.status]||last.status}`:'仅根据已记录动作显示，不推断未记录的思考。';
    ...
    const stream=$('tool-stream');stream.replaceChildren(...all.slice(0,3).map((a,i)=>{
      ...
      b.append(el('span','',String(i+1).padStart(2,'0')),el('strong','',a.tool),el('span','outcome '+a.status,a.status==='completed'?'结束':statuses[a.status]||a.status),el('small','',duration(a.durationMs)));
  ...
  function inspect(type){
    ...
    if(type==='decision'){para(root,'这些是 Agent 通过既有记忆工具保存的目标与复盘，可与工具回执核对；不是完整内部思维链。');para(root,summary?.summary.goal||'没有已保存目标');para(root,summary?.summary.lesson||'没有已保存摘要');para(root,'下一步：'+(summary?.summary.nextFocus||'未记录'));record(root,'Jev 快系统最近选择（独立历史记录，未关联当前轮次）',trace?.systemOne??{});}
