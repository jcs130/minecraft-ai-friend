...
1. **Wood**: `mine` 8+ logs (any `*_log`; hand works) → craft planks → sticks → a `wooden_pickaxe`. Prefer the one-call `craft` tool — it finds the recipe, lays the grid, takes the result, and reports exact missing amounts (`lookup_recipe` first if you want to check the ingredients). Manual grid-laying via `transfer` is the fallback when you want many-at-once batches (see the `containers` skill). 2×2 recipes (planks, sticks) use your own grid; a 3×3 (the pickaxe) needs a crafting table within 4 blocks — craft one first, place it (`build` op, or hold it and `interact_at button=right` on the ground), then `craft` again. Remember the table's coordinates (`remember_place`) and reuse it.
...
Checklist verified → mark phase 1 `completed` in 你的 goal(阶段清单记在 goal 里), then `read_skill("nether_entry")`.
