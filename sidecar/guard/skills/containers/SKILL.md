...
You have two ways to craft. **Try the one-call `craft` tool first** — it looks up the recipe, lays the grid (yours or a table's) and takes the result; for 3×3 recipes it needs a crafting table within ~4 blocks and its error tells you where the nearest one is. If it reports "no crafting menu opened" on a table you can see (some village tables are hijacked by settlement protection and open the wrong menu), craft your own table (4 planks) and place it: `build` op, or hold it and `interact_at button=right` on a ground block.

Manual grid-laying is the fallback, and the way to craft **many at once**:
...
- **Chest is full** → `inspect_gui` shows no empty container slots. Find another chest (scan / scan_blocks) or take something out first.
