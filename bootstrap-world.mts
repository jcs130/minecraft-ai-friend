...
import * as mcDiary from './src/mc-diary.ts'
import * as mcLoreSync from './src/mc-lore-sync.ts'
...
// 连接目标（Docker 部署时：MC_HOST=宿主或 mc-server 服务名；裸机默认 localhost）。
const mcHost = process.env.MC_HOST ?? 'localhost'
const mcPort = Number(process.env.MC_PORT ?? 25565)
const rconHost = process.env.RCON_HOST ?? mcHost
...
await ctx.plugin(mcLogwatch, {
  ...
  logPath: process.env.MC_LOG_PATH ?? 'C:/Users/lzl19/Documents/airi-minecraft/server/logs/latest.log',
  pollMs: 500,
})
await ctx.plugin(mcWorlddb, {
  ...
  memoryEnabled: true, // 众生册（Qdrant 独立 collection，与家里 MemOS 分开）
  ...
  embeddingModel: 'bge-m3-cpu:latest',
})
...
await ctx.plugin(mcGod, {
  enabled: true,
  qwenpawUrl: process.env.MC_QWENPAW_URL ?? 'http://127.0.0.1:8088/api/console/chat',
  cooldownMs: 60_000,
  pollMs: 12_000,
  ...
  advancementsDir: process.env.MC_ADVANCEMENTS_DIR ?? 'C:/Users/lzl19/Documents/airi-minecraft/server/advancements',
...
// 日记档案馆（2026-08-17）：每人每天一篇 → world.db + written_book 实物 + 城镇讲台。
// 协议走私聊（日记：…/日记本），与祈愿/信使通道互斥；讲台坐标空 = 不建。
await ctx.plugin(mcDiary, {
  enabled: true,
  bookEntries: 3,
  pageChars: 200,
  autoGive: true,
  lecternPos: process.env.MC_DIARY_LECTERN ?? '',
  lecternFacing: 'north',
})

// 世界记忆出口（2026-08-17）：world.db 编年史 → MemOS 公共知识库（mc-world cube），
// 让穿越者的 mc_lore / 夜间进化复盘都能检索到世界大事（穿越者进程零 RCON，只读公共库）。
await ctx.plugin(mcLoreSync, {
  enabled: process.env.LORE_SYNC !== '0',
  baseUrl: process.env.MC_MEMOS_URL ?? 'http://127.0.0.1:8002',
  cubeUser: 'mc-world',
  intervalMin: 30,
  batchSize: 20,
  maxPerRun: 200,
  statePath: './data/lore-sync-state.json',
  timeoutMs: 15_000,
})
