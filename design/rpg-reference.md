...
> 在千灯界永不可装**（除非举界迁 modded 客户端）。但它们的**机制设计**值得拆骨借魂，
...
另：**Passive Skill Tree / Puffish Skills**（Fabric 客户端）概念可参——**天赋树**。千灯界已有「出生天赋」（降临仪式），可扩为**天赋成长节点**：编年史里程碑（首猎/首祭/渡劫）点亮天赋枝，由女神裁定授位。
...
1. **可装册**：RPG/皮肤/特效三域，NeoForge 1.21.1 + 纯服务端约束下**无一可装**——这不是损失，是千灯界「自研为体、外部为参」路线的再确认。
