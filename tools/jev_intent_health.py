...
def check(root=ROOT):
    root = Path(root)
    checks = dict.fromkeys(('fresh_runtime', 'official_provider_ready', 'bounded_slots', 'tested_sources_current'), False)
    evidence = {}
    try:
        ...
        checks['tested_sources_current'] = (report.get('ok') is True and report.get('worldActions') == 0
            and report.get('testsPassed', 0) >= 7 and REQUIRED <= set(sources) and len(sources) < 20
