"""Stage one native Qwen backup for a predefined operations->game role migration.

Generalization of migrate_engineer_to_game.py for the five remaining operations
roles of the single-QwenPaw consolidation. The engineer migration stays on its
original frozen tool. Input must be the SHA-verified native export of exactly
one source role. The original signed archive is immutable; a separately named,
unsigned archive is produced for native import with explicit legacy trust.
Native restore has no ID mapping, so renamed targets are transformed here.
"""
...
from world_team_hosts import ENGINEER, MIGRATIONS  # noqa: E402

MAX_TOTAL = 2 * 1024 ** 3
MAX_FILE = 128 * 1024 ** 2
BASE_DRIVERS = {'qiandeng_operations', 'qd_learning', 'qd_world_team'}
WORLD_DAILY_JOB = 'qd-world-daily-default'


def entry_for(source_role):
    actor = 'operations:' + source_role
    entry = MIGRATIONS.get(source_role + '-to-game-v1') or next(
        (row for row in MIGRATIONS.values() if row['logicalActor'] == actor), None)
    if entry is None or actor == ENGINEER:
        raise ValueError('unknown_or_engineer_source_role')
    return entry
...
def expected_source_jobs(source_role):
    jobs = {'qd-learning-' + source_role}
    if source_role == 'default':
        jobs.add(WORLD_DAILY_JOB)
    return jobs


def workspace_inventory(folder, source_role):
    ...
    if profile.get('id') != source_role or set(profile.get('mcp', {}).get('clients', {})) != BASE_DRIVERS:
    ...
    return {'schema': 1, 'logicalActor': 'operations:' + source_role, 'files': files}


def archive_inventory(archive, source_prefix):
    ...
    for info in archive.infolist():
        ...
        if name != 'meta.json' and not name.startswith(source_prefix):
            raise ValueError('backup_must_contain_only_source_role')
        size += info.file_size
...
@contextmanager
def target_contract(entry):
    """Only this staging process sees the proposed final host policy."""
    with tempfile.TemporaryDirectory(prefix='qd-role-contract-') as temporary:
        path = Path(temporary) / 'runtime-hosts.json'
        path.write_bytes(encoded({'schema': 2, 'phases': {entry['migration']: 'active'}}))
...
def transform_profile(profile, entry):
    ...
    source_role = entry['source']['agentId']
    target_role = entry['target']['agentId']
    source_workspace = '/state/work/workspaces/' + source_role
    if profile.get('id') != source_role or profile.get('workspace_dir') != source_workspace:
        raise ValueError('unexpected_source_profile_identity')
    if set(profile.get('mcp', {}).get('clients', {})) != BASE_DRIVERS:
        raise ValueError('unexpected_source_driver_inventory')
    operations_cwd = profile['mcp']['clients']['qiandeng_operations'].get('cwd')
    if operations_cwd not in (None, '', source_workspace):
    ...
    result.update(id=target_role, workspace_dir='/state/work/workspaces/' + target_role)
    result = with_learning(result, target_role, 'game')
    # Identity move, not a runtime-policy upgrade: the existing policy must
    # still pass validate_learning_profile below unchanged.
...
def stage(source, output, expected_sha256, entry, source_inventory=None, prior_jobs=None):
    ...
    source_role = entry['source']['agentId']
    target_role = entry['target']['agentId']
    source_prefix = 'data/workspaces/' + source_role + '/'
    target_prefix = 'data/workspaces/' + target_role + '/'
    source, output = unlinked(source), unlinked(output)
    if not re.fullmatch(r'[0-9a-f]{64}', expected_sha256 or '') or sha_file(source) != expected_sha256:
    ...
    try:
        with zipfile.ZipFile(source) as archive, target_contract(entry):
            inventory = archive_inventory(archive, source_prefix)
            if source_inventory is not None:
                if (source_inventory.get('schema') != 1
                        or source_inventory.get('logicalActor') != 'operations:' + source_role
            ...
            ready = transform_profile(original, entry)
            ...
            if {j['id'] for j in original_jobs['jobs']} != expected_source_jobs(source_role):
            ...
            # Weekly and daily jobs keep their original logical meta, cadence,
            # session and history; no team shift belongs to these five roles.
            ...
            new_meta = meta.model_copy(update={
                'id': 'qd-' + target_role + '-migration-' + expected_sha256[:16],
                'name': 'Single operations role native host migration', 'signature': None,
                'accepted_via_trust': None,
                'description': 'Transformed local copy of ' + meta.id + '; source SHA256 ' + expected_sha256
                               + '; operations:' + source_role + ' -> game:' + target_role
                               + '; jobs and MCP initially disabled.'})
            target = output / ('qd-' + target_role + '-quarantined.zip')
            ...
            with zipfile.ZipFile(target) as verify:
                if any(n.startswith(source_prefix) and source_prefix != target_prefix
                       for n in verify.namelist()): raise ValueError('old_native_id_in_archive_path')
            ...
            report = {'schema': 1, 'ok': True, 'phase': 'staged-not-imported',
                ...
                'logicalActor': 'operations:' + source_role, 'migration': entry['migration'],
                ...
                'targetRole': target_role, 'targetWorkspace': '/state/work/workspaces/' + target_role,
                'changedWorkspaceFiles': sorted(changes),
...
if __name__ == '__main__':
    ...
    try:
        entry = entry_for(args.source_role)
        if args.inventory_workspace:
            ...
            result = workspace_inventory(args.inventory_workspace, args.source_role)
        ...
        result = stage(args.source_backup, args.output, args.expected_source_sha256, entry,
        ...
        print(json.dumps({key: result[key] for key in ('ok', 'phase', 'archiveSha256', 'targetRole', 'changedWorkspaceFiles',
