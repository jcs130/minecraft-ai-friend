...
LABELS = {'game:mc-god': '灯语女神', 'operations:mc-god': '天神 · 世界工程师',
...
def configure(mode='preview'):
    ...
    for actor in inventory:
        ...
        if mode == 'preview':
            journal.append({'actor': actor, 'drivers': list(bindings(role, runtime)), 'cron': actor in SCHEDULES})
            continue
        ...
        if mode == 'files':
            ...
            api(runtime, 'PUT', '/agents/' + role, role, {'id': role, 'name': LABELS.get(actor, before['name']),
                'tools': proposed['tools'], 'security': proposed['security'], 'approval_level': proposed['approval_level']})
            ...
            api(runtime, 'PUT', '/agents/' + role, role, {'id': role, 'name': current['name'], 'mcp': mcp})
        ...
        for key in ('id', 'workspace_dir', 'active_model', 'fallback_models', 'running'):
            assert before[key] == after[key], 'unrelated_role_change:' + key
        journal.append({'actor': actor, 'phase': mode, 'verified': True})
