...
KNOWN = frozenset(('mc','world','gate','npc','resources','qwenpaw','voice','asr','panel','tts','control'))
...
def collect_snapshot(root=ROOT):
    ...
    for row in registry['services']:
        ...
        rows.append({key:row[key] for key in ('id','label','group','purpose','dependencies')}|
                    {'container':name,'state':state['state'],'health':state['health'],'managedBy':'D 项目 Compose · '+str(state.get('restart') or '无已知守护')})
    ...
    return {'schema':1,'project':'qiandengji','generatedAt':utc(),'runtimes':runtimes,'agents':agents,'services':rows,'issues':issues,
...
def lifecycle_plan(action,selected,states):
    ...
    stop_order=[n for n in ('npc','world','gate','voice','asr','qwenpaw','resources','panel','control','mc','tts') if n in expanded]
    ...
    start=[n for n in ('tts','mc','world','gate','npc','qwenpaw','resources','voice','asr','control','panel') if n in start] if action!='stop' else []
...
def main(argv=None):
    ...
    parser.add_argument('--group',choices=['all','game','voice','dialogue','management'])
