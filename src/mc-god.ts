...
export function createGod(config: Config, deps: GodDeps): GodHandle {
  ...
  async function initNewcomer(username: string): Promise<void> {
    ...
    const prompt = [
      ...
      '4. 指路：把世界的「命令接口」私聊告诉他（2026-08-23 定稿）——已习得的技能自己咏唱：私语 /msg Goddess 念法术关键词即可（归乡/圣愈/造物/照明/传送…）；咏唱可带咒语框架前缀（如「' + prefixHint + '」+ 法术内容）更显仪式，前缀只知一二，其余藏在技能书里待他寻访；求助私聊「祈愿：…」可自愿献供奉；疑问私聊「问：…」；选天赋喊「我选 <法术名>」；查状态说「鉴定」；/help（或打 cli）查生存手册。',
      '',
  ...
  async function answerQuestion(username: string, question: string): Promise<void> {
    ...
    try {
      ...
      const playbook = [
        ...
        '- 命令接口=CLI（本神御定的命令书）：凡「操作/施法/求物/选天赋/交易」直接按命令办——私语念词 / 祈愿： / 问： / 交易：<物> 给<玩家> / 我选 <法术名> / 鉴定；查命令书打 cli 或 /help。',
      ].join('\n')
      ...
      let prompt = [
        ...
        ...(isAi ? ['提问者是 AI 穿越者（读不了书）：多直接给命令（念词/祈愿：/问：/交易：/cli），少讲叙事。'] : []),
