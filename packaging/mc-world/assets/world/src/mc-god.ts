  ...
  async function answerQuestion(username: string, question: string): Promise<void> {
    ...
    try {
      ...
      const playbook = [
        ...
        '- 命令接口=CLI（本神御定的命令书）：凡「操作/施法/求物/选天赋/交易」直接按命令办——私语念词 / 祈愿： / 问： / 交易：<物> 给<玩家> / 我选 <法术名> / 鉴定；查命令书打 /help；AI 玩家读不了书，可打 /cli 看命令接口。',
      ].join('\n')
      ...
      let prompt = [
        ...
        ...(isAi ? ['提问者是 AI 穿越者（读不了书）：多直接给命令（念词/祈愿：/问：/交易：），指它打 /cli 看命令接口、/help 看手册，少讲叙事。'] : []),
