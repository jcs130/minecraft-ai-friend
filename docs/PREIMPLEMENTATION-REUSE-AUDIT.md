...
审计日期：2026-09-20。第 1–11 节的运行统计固定到北京时间 08:53:28，属于实施前审计；后续补查技能资格与隔离 fixture。用户授权后的首轮实施记录见第 12 节，不能用后来的结果替换审计时的事实。
...
## 12. 2026-09-20 首轮 RSI 控制接入

本轮复用 Numen 原生输入任务、`WorldInteractionBridge` 持久回执、原身体租约及 QuickJS 技能库，未增加执行服务、模型循环或玩法专用执行器。

- 开放 `interact_at` 给 QwenPaw 和程序技能：左右点击、明确目标或全 null 沿当前视线、可选实际携带物品、0–100 tick 的持续使用。MCP 共 47 项，程序动作共 18 种。原生触及距离、物品消耗、身体所有权与保护区域继续有效；食物仍走原 `eat`。取水只是隔离验收用例，没有新增硬编码取水工具。
- 通用交互受理后立即交还调度；控制器查询同一 request / body / epoch / native task 的原回执。重启、响应丢失及终态查询均不重新点击。交互完成与技能目标达成仍分别判断。
- RCON actuator 使用 Numen 的 `EXTERNAL_CALL_PREFIX`，真实异步回执改为外部轮询指引，避免承诺调用方收不到的内置脑事件。这不等于实现了新的通用完成事件总线。
- `skill_start` 在关闭租约、创建任务前检查当前内核的测试资格；同时修复 `SkillTools.library` 缓存初始化的缩进错误。生产原 `craft_sticks`、`farm_harvest_replant` 各 7 个 fixture 真实重跑通过，旧报告备份，源码、activeVersion 和晋升历史不变。
- 使用现有 MCP 配置工具增补单个工具许可和实际连接清单；教程放入原 `program-practice.md` 按需查阅。现有行为会话、增量上下文继续复用，未加长全局 Agent 提示词。

验证：survivor 部署镜像内 180 项相关 Python 回归通过；Java 47 项契约检查通过；全新隔离世界使用现役 72 个模组快照，40 项实机断言通过，耗时 422.53 秒。实机覆盖空桶取水后的背包变化、40 tick 持续使用后释放、普通放置与真实消耗、丢失响应只查询、重启后终态保留和未完成请求不重放、外部异步回执。隔离测试没有模型调用，不能解释为 Agent 自主学会了这些操作。

本机证据位于忽略目录 `runtime/interaction-qa-32fe0a966ae9/result.json` 和 `runtime/rsi-native-controls-20260920/`。部署在原任务及动作终态后暂停，逐文件校验备份生产世界的 19,663 个文件（约 2.19 GiB）。Numen 主体 0.1.3 保持原 SHA256；此次构建/安装的是 Iron 桥 `321c2e5c…` 和 actuator `9271e615…`。`server-extensions.lock.json` 只更新本轮桥及 actuator 产物；旧 Numen 来源记录漂移没有被伪装成重新验证过的构建。

生产恢复验收：原生健康探针 7/7 通过，原生 QwenPaw handler 的 47 项工具全部启用，`interact_at` 参数结构核对通过。原身份和背包保持一致；恢复原自主开关、NPC 准入及原本启用的 13 个 Cron（其余 3 个继续关闭），随后观察到桐人正常进入新模型回合。四个重启服务继续由原 Docker `unless-stopped` 策略守护，未重启 QwenPaw 或重写会话历史。

完整 `probe_panel_smoke` 已执行，本轮 survivor、原生交互检查通过。发现导航探针仍单独写死 Numen 0.1.1，改为复用交互探针的现役依赖路径，5 项探针测试通过且生产查询通过。实践报告过期则在无网络容器中对**现役挂载源码**重新运行原 `smoke_survival_practice.py`，34 个案例通过（与前述 180 项有重叠，不相加），保存新报告后 live / behavior 探针均通过。全项目面板汇总仍有配置漂移和历史证据失效红灯；没有修改旧报告的哈希或成功标记来凑绿。

维护时发现现役原生恢复仍有缺口：Minecraft 重启后没有自动恢复主人离线的桐人，注册表显示旧死亡记录。只读比较备份与现役 playerdata 确认生命值均为 20、UUID/位置/背包均与停机前一致后，记录意图并调用一次已有 `numen_restore_existing` 接回原 UUID；上线后再次核对完整背包相等。没有新建分身、恢复旧存档或修改死亡记录来伪造成功。此操作恢复了本次运行，但不代表主人离线时的自动恢复机制已修好。

尚未完成：原始 WASD/视角控制、聊天与身体动作准入解耦、同场景性能基准、Agent 自主改进后在保留场景上的收益验证。QwenPaw 既有学习任务 `text_drift` 与结衣旧生活输入挂起状态仍需单独处理；本轮保留这些状态，不写入虚假的成功结果。接口开放和旧技能重新具备运行资格只是 RSI 的第一步。
