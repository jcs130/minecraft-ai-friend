...
> 本文为接口拆分阶段记录。后续旧 9090 游戏面板已停用，19091 已加入天神之眼及经过登录和预览确认的服务器管理；最新使用入口见 [服务器管理](SERVER-MANAGEMENT.md)。QwenPaw 的可替换边界仍保持。
...
| D 盘 QwenPaw 控制台 | http://127.0.0.1:18089 | 使用原登录方式管理 QwenPaw 内部模型、角色与对话 |
| 原 C 盘管理页 | http://127.0.0.1:9090 | 原 shadow-panel 仍占用该端口，本轮保持原服务 |
