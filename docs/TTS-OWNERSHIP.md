...
**2026-09-13 更新：** 当前硬件为 RTX 3090 24GB，生成式 LLM 全部使用云端。游戏语音先重建 IndexTTS，随后按用户要求切到 C 盘现存的 Kokoro，短句完整响应中位约 0.16 秒；当前部署与回滚见 [Kokoro 游戏语音](KOKORO-TTS.md)。旧 Docker 镜像已不可见，新增的 `qiandengji-tts:2.5-qd1` 与独立运行证明作为回滚保留；下文旧镜像 ID 和 9 月 7 日验收为历史记录。云端分工见 [本地语音与云端 LLM](LOCAL-SPEECH-CLOUD-LLM.md)。
