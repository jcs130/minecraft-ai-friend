...
2026-09-08 后续技能整合：六角色各保留原职责与证据技能，另启用技能学习流程及 QwenPaw 官方 make-skill、file_reader、cron，共36项基础绑定。普通流程优先原生创建与市场复用。周复盘由原生调度器管理，沿用共享4次/24小时预算；没有新证据不调用模型。下文的12项绑定、工具全部关闭及周期任务关闭是先前阶段记录，当前边界见 [角色学习](ROLE-LEARNING.md)。

2026-09-07：独立运营服务已从 QwenPaw 2.1.0 升至 **2.2.0**。官方版本说明：[v2.2.0](https://github.com/agentscope-ai/QwenPaw/releases/tag/v2.2.0)。该版改进后台协作任务跟踪、取消和模型用量统计。该次版本升级未涉及游戏会话服务 18089、Minecraft、旧角色身体和存档；宿主 QwenPaw 8088 保持原用途。

日常入口：[运营组](http://127.0.0.1:19091/#operations)。工作区、对话、原生后台任务和技能管理：[运营 QwenPaw 控制台](http://127.0.0.1:18090)。按 2026-09-08 用户要求，管理台 19091、游戏 QwenPaw 18089、运营 QwenPaw 18090 均本机免密码访问，仅发布到 `127.0.0.1`。无需读取登录文件；打开运营控制台后选择司灯即可按需发起任务，例如“让灯语检查当前快照，给出最值得处理的一项问题”。

此前控制台凭据仅为兼容已有机器调用保留，仍在忽略的私有目录中，不公开或提交。本轮授权更新并重建游戏 QwenPaw 的登录环境变量，保持其版本、模型和角色；不重启 Minecraft、world 或宿主 QwenPaw。管理台内部 control Bearer、CSRF、Host/Origin 校验及维护预览确认继续保留。免密码访问不改变下面的模型限额，也不新增模型任务。
