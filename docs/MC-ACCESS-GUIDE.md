...
> **2026-09-21 重要更正 ✗**：本表"能用"仅指**连得进**；mineflayer 直连 25565 时 **方块/物品读数错乱未愈**
> （NeoForge 网络号与原版表不一致 ✓ 详见 §六 进行中修复）✓ 接 Agent 请优先走 **gate:25701** ✓
...
|---|---|---|---|
| **AI / 机器人（mineflayer 等）** | **首选 `127.0.0.1:25701`（gate）** ✓ 兜底 `<服务器IP>:25565` | 无需正版账号；`auth: 'offline'`；协议钉 `1.21.1` | **连得进 ✓；读数：经 gate 物品已翻译 ✓ 方块仍错（§六 修复中）** |
