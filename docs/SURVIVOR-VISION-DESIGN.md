...
此阶段提供**局部语义俯视图**，半径4–12格，北上东右，一格对应一格水平位置。网格来自服务端已加载数据；高度已经由 Numen 合并为通行类别，不含材质、精确高度、实体叠层或视觉遮挡。图片是真实观测的可视化，**不是 FOV110 第一视角截图**。完整体素快照、切片和本人眼位透视图仍是后续设计，不能从现有字符格推造。

采集校验本人绑定、维度、前后位置与原生格式；失败返回无图。PNG 编码缓存最多两帧，每次仍重新读取世界，旧图不会被重新标为新观测。15项图像模块测试已在原 survivor 镜像的断网、只读隔离环境通过；另一次未部署的内存加载验证，从现服只读观测生成620×616 PNG，未移动身体、调用模型或写入容器文件。这些证明采集与编码可用，**不等于生产工具已部署、Qwen 已自主看图或供应商端已完成图像推理**。

23:19 已部署原 survivor autonomy23，原Qwen MCP重载为46工具，原生认证HTTP已实际返回PNG，当前Qwen2.2.1格式化保留图片的测试通过；两个自然生活回合已恢复完成。上段未部署的内存实验保留为历史记录，云端读图验收与当前运行记录详见 [多模态与限流恢复实现](SURVIVOR-MULTIMODAL-RECOVERY.md)。本次同时修正原循环对供应商短时限流的分类与恢复方式。

## 2026-09-08 调研存档

以下为当时的设计与验证记录；其中工具数量、部署状态和“尚未实现”的描述均以2026-09-08为时点。当前实现状态以上方更新及后续验收记录为准。用户要求多模态图像输入、FOV110，并明确认为常驻完整客户端太重，因此调研采用服务端优先方案。
