...
| 用途 | 本机入口 |
|---|---|
| 游戏客户端直连 | `127.0.0.1`（默认 25565）；原 `127.0.0.1:25567` 仅保留本机兼容入口 |
| 同一局域网客户端 | 当前 `192.168.3.133`（默认 25565；以太网 3，IP 改变后需更新客户端地址） |
| Agent 协议入口 | `127.0.0.1:25702`（外门 ✓ 本机与局域网统一 ✓ 名须 ag_）|
...
游戏 TCP 25565 与 Simple Voice Chat UDP 24455 已发布到 `0.0.0.0`，允许局域网连接；管理台、QwenPaw、RCON、Agent gate、资源分发和 TTS 仍仅发布在本机回环地址。原 TCP 25567 只绑定回环，供既有宿主脚本继续使用；容器间仍连接 `mc:25599`，不把容器内部端口改成玩家入口。旧运营 18090 不作为日常入口。Docker 网络重建后须核对管理台实际来源，当前 `qiandengji_default` 网关为 `172.18.0.1`；不能继续使用旧 `172.20.0.1` 而导致免密码会话被拒绝。

其他电脑须使用本项目的 Java 1.21.1 / NeoForge 整合客户端，在“多人游戏 → 添加服务器”输入 `192.168.3.133` 或 `192.168.3.133:25565`。此独立服务没有局域网自动发现广播，不能以扫描列表是否出现判断连接是否可用。Simple Voice Chat 的 `voice_host=24455` 只指定外部端口，主机地址沿用玩家实际游戏连接，兼容本机与局域网；旧 `127.0.0.1:24455` 会把其他电脑的语音错误地指向它自己。客户端已预装的资源包继续随整合包使用，不将免密码管理页面开放给局域网。

本机 Private 网络已有 Docker Desktop Backend 的 TCP/UDP 入站允许规则，本次未修改防火墙或关闭防火墙。重建主服前已停消费者并取得 `save-all flush` 成功应答。验收入口为 `python tools/check_lan_access.py 192.168.3.133`：从服务器本机经 LAN 地址进行真实 Minecraft 状态握手，并核对只有游戏/语音端口对外发布；它不代替另一台电脑的真实进服或语音播放验收。Simple Voice Chat 端口语义见[官方配置说明](https://modrepo.de/minecraft/voicechat/wiki/server_config)。

实机本机默认 `127.0.0.1:25565` 已成功返回 1.21.1 / 协议 767。上述本机 LAN 回连诊断仍记录超时：当前 WSL 为 `networkingMode=mirrored`，没有启用默认关闭的 `[experimental] hostAddressLoopback`，本机及同一 WSL 中容器访问宿主自己的 LAN IPv4 可能受此限制。不能把这次超时改写成成功，也不能因此断言另一台局域网电脑不可连接。本次未为自回连测试重启全局 WSL 或启用额外代理。见[微软配置说明](https://learn.microsoft.com/en-us/windows/wsl/wsl-config#experimental-settings)及[同类已解决案例](https://github.com/microsoft/WSL/issues/41284)。本机使用 `127.0.0.1`，其他电脑使用 `192.168.3.133`。

2026-09-13 用户已在“同一局域网另一台电脑的 Java 整合客户端手动添加 192.168.3.133”验证问题下回复“能看到服务器并连接”。因此外机连接有用户实测确认，和上述本机自回连超时分别记录；没有将用户连接确认冒充自动化登录或语音听觉测试。回执为 `runtime/lan-access-20260913/external-client-confirmation.json`。
