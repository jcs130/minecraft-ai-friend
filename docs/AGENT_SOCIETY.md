...
居民不是只等村长派活。正确控制方式是“村长给方向，居民自己循环”：每个居民通过 Mindcraft `!goal(...)` 获得长期职业目标，控制台在空闲时补一个短动作心跳，村长只在全局优先级、安全兜底和项目协作上介入。`!newAction(...)` 会触发代码生成，可以作为复杂动作、高级建造、复合采集或脱困的受控能力。更详细的 Mindcraft 用法见 [MINDCRAFT_USAGE.md](MINDCRAFT_USAGE.md)。
