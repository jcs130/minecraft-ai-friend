...
- **准入闸现状：`white-list=false`（2026-09-22 造物主裁量「人数有限」）** ✓ `ag_` 名即可进 ✓ 名单 22 号保留 ✓ 要收紧随时 `whitelist on`（曾实测：开闸时未列名 `ag_` 名被拒 ✓是无白名单直接进来的 ✓ 已退出 ✓）
  → **新名字上线流程**：`whitelist add <ag_名>`（RCON 即时生效 ✓ 不用重启 ✓）→ 再连 25702 ✓
