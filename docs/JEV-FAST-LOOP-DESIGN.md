...
研究日期：2026-09-21；下文核查基线为 d2068ea，保留当时观察。**同日已部署第一阶段快慢控制**：异步单槽、连接复用、状态绑定/过期拒绝、任务局部 context 与原回执衔接，见[实现与实测](JEV-FAST-SLOW-CONTROL.md)。下面更广的批量语义分类、任务内抢占和原生输入片段仍属设计，不能将其全部视为已实现。

同日进一步克隆核查了7个驾驶、无人机、机器人与Minecraft控制项目，补充见[控制项目源码对照](JEV-CONTROL-REFERENCE-REVIEW.md)。设计据此扩大：Jev既能判断继续/修复，也能从带局部预计效果的联合运动候选中选短动作；优先借鉴JevPilot的候选投影和版本绑定、Minecraft的有界输入及执行复核。各项目存在仿真暂停、特权感知或未通过的测试，不能直接照搬频率和成功率。
...
进一步在当前阶段提供少量联合候选 `(primitive, targetId, boundedSegment)`，附局部计算的预计终点/触达/进度、占位危险与未知项，供Jev选择具体短行为。不要把“是否继续”混进大量近似运动候选而改变其概率含义；可将继续/停止与“假定继续选哪个片段”拆成同批条件式问题，再由代码核验组合。默认选一个完整候选；无人机示例的概率加权只作为可组合连续控制轴的研究方向，不平均不同路线、对象、攻击与交互。候选生成器亦可由Agent修订，经技能测试、独立任务评价后晋升。
