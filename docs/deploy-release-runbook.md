...
1. 宿主：生产检出同步到发布对象（基线锚）1659453d19e44320872841dd15bdbfe4c84cf061；锚之后的提交若仅触 docs/ 与 tests/（文首后缀规则），可直接检出分支 HEAD，运行时字节与锚等价。
   注意：队列 15 笔均 pushed=false（推送通道缺失即本工单主题），生产检出 `git fetch` 远端**拿不到这些提交**；
   ...
   回执：`git rev-parse HEAD`（应=1659453…；按后缀规则检出 HEAD 时回执实际值，并附 `git log --name-only --pretty=format: 1659453..HEAD | sort -u` 自验输出）+ `git status --porcelain`（应为空）。
