...
后续实现已进入部署阶段，当前结构、原生工具与限制见 [女仆桥接实现](MAID-BRIDGE-IMPLEMENTATION.md)。以下保留最初的只读核查，不能再把“本轮没有部署”当作当前运行状态。

后续组队需求见 [桐人与女仆旅行伙伴](SURVIVOR-MAID-PARTY.md)：优先研究专属伙伴通过原生认领复用跟随，两位独立 Qwen 角色通过各自持久会话协作。队伍通信和同游尚未部署，不改变已有真人女仆的归属。
