...
2026-09-20：实服与仓库目录已统一为 35 个精选入口：9 项女神秘术（下表 8 项加星辉术 `starlight`）和 26 个铁魔法主题别名，后者对应 21 个原生法术。别名复用真实装备、原生魔力及冷却，不代表额外学会 26 项能力，也不是旧动漫效果全部复活。37 项历史档案（含 7 被动）继续保留；以 `help <ID>` 和实际回执为准。
...
右键带 ✦ 标记的技能指南针，或 F6，打开六行技能罗盘。默认“我的技能”只列已学/天赋秘术与实际装备的原生法术；顶栏按战斗、辅助、移动、生活、被动和铁魔法分类，右侧提供指南和传送阵。底部“学习图鉴”单独展示可尝试学习项及未满足条件，另有档案、快捷栏、刷新与翻页。档案仅展示旧技能及原生替代说明，不会点击施放或发放物品。F7 仍为千灯手札。普通指南针不带技能标记时不被接管。

- `/mycli spells`：查看自己实际装备的原生法术。装备法术书或手持卷轴后才能使用。
- `/mycli spells legacy`：查看当前 35 个精选入口及其执行引擎；`/mycli spells archive`：查看 37 项历史档案。
...
八槽快捷配置与罗盘每页 28 个技能展示位分别使用。已有归档技能绑定在视图中空置，原始绑定记录保留；重新配置该槽才明确替换。被动能力不占主动槽。
...
罗盘右上角指南，或 `/mycli menu guide`，查看卷轴、铭文台、法术书与装备步骤。`/mycli help irons` 提供同样的文字入门。铭文台将卷轴法术写入书，装备法术书后可以重复施法；手持卷轴可一次性施法。技能预览不发物品、不豁免原生条件。来源：[Iron 官方进阶说明](https://iron.wiki/progression/)、[法术书说明](https://iron.wiki/spellbooks/)。

- `/mycli skills --json`：简短能力索引；主题别名分列，不重复计为已学秘术。
- `/mycli help tp --json`：单项条件、参数、成本体系与下一步。
- `/mycli help rasengan --json`：主题别名的实际原生映射与当前装备状态。
- `/mycli spells --json`：实际装备快照；灰色项注明原因，恢复后刷新。

`ready` 只是打开/查询时的预检；目标、距离、事件取消和实际效果须读执行回执及后续感知。`casting_started` 不等于命中成功。女神秘术与铁魔法使用不同资源池；映射项的旧 `cost/level` 不适用，JSON 返回 null 并标明引擎，不能拿旧数值指导施法或调参。
