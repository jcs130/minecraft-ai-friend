...
2026-09-08 调研、设计与实施记录。稳定生活会话、连续动作和游戏内伙伴交流已部署，真实连续生活结果以本机 smoke 报告为准。下文“本机查到的差异”保留改造前的研究证据，不代表当前运行配置。

## 设计结论

桐人与女仆组队的接入见 [旅行伙伴方案](SURVIVOR-MAID-PARTY.md)：各自保留生活主会话，游戏确认发言被听见后，经现有桥串行接收伙伴消息。稳定身份、取消与统一串行入口已部署；人格和个人资料分别保存。
...
## 当前实现与验收边界（2026-09-08）

候选源码已实现持久生活会话和同一 task 的连续工具行动。`life_session.py` 在 `life-session.json` 保存一次生成的主 session、固定 `survival-controller / console` 及身体 UUID；`controller.py` 为每次唤醒另建唯一 turn 与原生 task，保存并核对实际 chat UUID。已知 task 的控制器重启只读接续轮询；丢失 POST 回复、原生 task 404 和未知动作不会自动重投。旧聊天与已有决策、用量记录保留，首次主会话通过已有笔记和事实接续，不伪造历史消息。

暂停立即关闭身体租约。QwenPaw 2.2 原生 stop 只能按 chat 停止，不能原子地绑定某个后台 task；为防止停止已接续的新任务，当前实现不以 session 或 chat 猜测取消，而是只读等待指定 task 的终态或原生超时。此时保留 active 与取消待确认状态，不能描述为“模型已停止”；未知 task 仍需核查。

模型工作租约允许最多6个串行动作，程序步骤仍一次一个。每个动作保存唯一 actionId、自己的 before/after、原生返回和独立回执；`inflight-action.json` 与进程间互斥维持一个身体动作在途。`goto` 终态必须匹配原生 taskId 和 navigation epoch，不能靠位置或 idle 推断成功。原生采矿尚无同样的持久终态接口，空闲时只记 `observed_ended / completionConfirmed=false` 和实际物资变化。法术 `casting_started` 没有 Numen taskId 时记 `effect_unconfirmed`，结束本次直接动作窗口，不宣称命中或施法效果完成。

Agent 可以选择 `status(wait_seconds=0..10)`：默认立即只读，有在途动作时最多每2秒采样一次，出现终态提前返回；等待结束仍在途就让本次工作结束，之后同一会话再接续。最后一个持久回执不会因后台先读取而消失。6动作是执行上限，当前12次模型迭代包含查询、行动和回复，提示预留最后两轮收尾，不承诺每轮实际能完成6动作。

每次唤醒输入缩为目标、必要身体状态、未处理事件和回执摘要；地形、库存、配方、公会和技能由 Agent 按需调用 MCP 或读取原生笔记。伙伴消息通过同一控制器入口预约，忙时留在队列，并复用当前预算与租约；提交前核对当前绑定的角色、身体、session、user、channel和伙伴配置 revision。收到实际完成的原生 assistant `message` 才形成答复；reasoning、工具输出、进度和未完成文本不能作为伙伴最终答复。

回归测试位于 `tests/test_survival_life_session.py`，覆盖同会话跨 task/控制器实例、未知请求不重放、精确任务等待、逐动作归因、在途屏障、伙伴预算与绑定、只读等待，以及安装 Qwen 原生消息结构的解析。测试运行于隔离网络的既有 Qwen 镜像和临时状态，不调用生产模型或修改世界。它们证明程序边界，不证明在线重启、长上下文压缩或长期生活效果。

只读验收工具为 `tools/smoke_survivor_life.py`。启用新一轮工作前运行 `baseline`，保存明确时点、已有 turn、原生本人用量和日志位置；之后 `collect --before <baseline>` 仅 GET Qwen 原生 task/chat/用量并读取本地动作账本。通过条件包括至少两个基线之后的新原生完成 task 证实同主 session 和固定 user/channel，以及至少一个该窗口内有独立前后状态的原生身体动作回执。task 404、未知副作用和倒退/缺失用量不会假绿；报告明确列出未执行的在线重启项，并且不保存聊天正文。

运行报告落在忽略提交的 `reports/survivor-life-smoke.json`。2026-09-08 21:24 后的独立新窗口已经实机通过：两个不同原生任务在同一生活 session/chat 完成正常最终答复，五份原生步行动作回执具有匹配身份、独立前后状态和导航 task/epoch，无未知动作；两次主动伙伴发言均有真实游戏 heard 记录。窗口内桐人原生计数增加16次模型请求、1,822,512输入token、3,427输出token，仅为原生计数差，不换算人民币费用。此前迭代耗尽、观察失败及误发系统提示的试验保留，但不计入此次通过。在线任务跨重启续跑、长上下文压缩和长期建造/生存效果仍未由这个短窗口证明。研究时48次规划是历史值，现96次/24h，不将它称为模型套餐余额。
