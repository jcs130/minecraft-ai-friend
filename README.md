...
QwenPaw 已作为默认适配器保留，会话调用支持注入替换实现。打开 [日常管理首页](http://127.0.0.1:19091)：天神之眼、服务维护、运营组、技能档案、村务和共享传送点统一入口。管理台 19091、游戏 QwenPaw 18089 和运营 QwenPaw 18090 均本机免密码访问，仅发布到 `127.0.0.1`；宿主 8088 保持原用途。服务器管理保留维护预览确认、日志、执行回执及内部鉴权，原网页登录凭据仅为兼容机器调用保留。最新使用与架构见 [服务器管理说明](docs/SERVER-MANAGEMENT.md)。
