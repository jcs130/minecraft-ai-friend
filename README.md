...
系统 1 首阶段已接通本地 Jev 兼容 Decider：已测试程序生成少量完整动作候选，本地模型依据新鲜局部状态选择，原身体网关执行与验证；每步不重传历史思考，故障和低置信度交回 Qwen。真实身体影子推理已验证，尚未证明逐帧 WASD 控制或策略收益。实现与部署证据见 [原生恢复与系统 1](docs/SYSTEM-ONE-NATIVE-CONTINUITY.md)。
