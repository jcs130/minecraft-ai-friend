...
[边玩边聊的调度设计](docs/EMBODIED-SOCIAL-SCHEDULING.md)已对照 Cortico 的合批/有限抢占、Neko 的聊天与任务分流，以及具身交互的话轮研究：将“何时回应、是否改变身体行动、是否形成承诺”分开，复用现有消息账本、只读交流与 QwenPaw 原生任务。当前交流仍会等待普通规划，目标入口也还不是多目标队列；语义调度、承诺议程与全双工语音是待实施方案，不代表已经上线。
