...
## 二、服务与基础设施（15 个默认受管服务）

| 服务 | 镜像 | 职责 | 宿主端口 |
|---|---|---|---|
| `mc` | itzg/minecraft-server:java21 | MC 服务器，持 shadow 世界存档 | 25565(LAN) / 25567(本机兼容·**裸口不过门**) / RCON 25577 / 语音 24455·udp |
| `world` | qiandengji-world | 房主女神世界引擎 + 天神之眼渲染（**数据面走门**：`MC_HOST=gate` + `MC_GATE_TRANSLATED=1`） | 19092 |
| `gate` | qiandengji-world | vanilla↔NeoForge 握手代理 **+ 号翻译（内门）** | 25701 |
| `gate-public` | qiandengji-world | **外门**：只收 `ag_` 前缀名的外部 Agent（握手 + 号翻译同上） | 25702(LAN) |
| `numen-mcp` | qiandengji-qwenpaw-game | numen 假玩家身体工具 MCP（HTTP + Bearer）；宿主 stdio 已退役 | 18091(仅本机) |
...
> `qwenpaw-ops`（旧独立运营实例，18090）在 compose 中保留定义但**已归档、不在 15 个活动服务内**；运营角色已并入游戏 QwenPaw 18089。
...
**外部 Agent 接入**：**一律走门，不要直连 `25565/25567`**——那两个是裸口，NeoForge 重排过 blockstate/item 号，直连会读到**错位世界**（红床→活塞、obsidian→火、模组物品空白），Agent 的寻路/挖掘/避怪全部失真。
- 本机/容器内 Agent：**内门 `127.0.0.1:25701`**；外部 Agent：**外门 `25702`**（名字必须以 `ag_` 开头 ✓ 内部号冒名在门口即拒）✓ 协议钉 `1.21.1`、`auth: offline`
- numen 假玩家身体工具：走 **`numen-mcp` 容器服务**（HTTP + Bearer ✓ 宿主 `127.0.0.1:18091`）✓ 宿主 stdio 桥已退役（`tools/run_numen_mcp.py` 仅作旧路径保留）
- 号映射与验证：见 [BOTGATE-IDMAP](docs/BOTGATE-IDMAP.md) · Agent 手册：[AGENT-ONBOARDING](docs/AGENT-ONBOARDING.md) · numen 细节：[NUMEN-MCP](docs/NUMEN-MCP.md)
...
| 用途 | 地址 |
|---|---|
| **Agent 接入（内门·过号翻译）** | 127.0.0.1:25701 |
| **Agent 接入（外门·名字须 `ag_` 前缀）** | 192.168.3.133:25702 |
| 局域网真人游戏连接（装 NeoForge 的客户端） | 192.168.3.133:25565 |
| 本机兼容裸口（**Agent 禁用 ✗ 不过门会读错世界**） | 127.0.0.1:25567 |
| 基岩版（手机/Switch/Win10&11） | 192.168.3.133:19140/udp（宿主 ViaProxy·已过门） |
| 基岩容器 A/B 试验口 | 192.168.3.133:19141/udp（`qd-geyser-ab` ✓ 待手机验证） |
| numen 身体工具 MCP（HTTP+Bearer） | http://127.0.0.1:18091/mcp |
| 本项目 RCON（**管理面不绕门**） | 127.0.0.1:25577 |
...
> 号映射与门的关系、以及"为什么 Agent 必须走门"：见 [docs/BOTGATE-IDMAP.md](docs/BOTGATE-IDMAP.md) ✓
> 不在 compose 里的宿主服务（基岩桥 / 皮肤代理 / Geyser 试验）正本：见 [world/host-services/](world/host-services/README.md) ✓
